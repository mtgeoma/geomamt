...
int main(int argc, char *argv[])
{
	...
	case CommandLine::mackie:
		...
		printf("%-14d%6.4f\n",J.size(),1.0);
		// for(int i=J.size()-1; i>=0; i--) { // por que fiz isto?
	...
	case CommandLine::dist_types:
		for(size_t i=0; i<J.size(); i++) {
			...
			double beta3=J[i].beta(3); // com alfa1
			double beta4=J[i].beta(4);
			double beta5=J[i].beta(5); // com alfa2
			double beta6=J[i].beta(6);
                        cout.setf(ios::scientific);
			cout<<setw(entrada.width())<<setprecision(entrada.precision())<<T
				...
				<<setw(entrada.width())<<setprecision(entrada.precision())<<beta5
				<<setw(entrada.width())<<setprecision(entrada.precision())<<beta6
				<<endl;
		}
		break;
