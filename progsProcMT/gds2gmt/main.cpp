...
int main(int argc, char* argv[])
{
    ...
    enum output_data {Rzx,Izx,Rzy,Izy,Azx,Pzx,Azy,Pzy,Riv,Iiv,Tzx,Tzy};
    ...
    if (argc<4 || argc>5) {  // wrong number of parameters
        cerr<<"usage: "<<argv[0]<<" -I<input data> -F<format data> -D<output data> [-R<angle>]"<<endl
            ...
            <<"         Tzx:"<<endl
            <<"         Tzy:"<<endl
            <<"         in the format: period real imag error"<<endl<<endl
        ...
        for(int i=1; i< argc; i++) {
            if(argv[i][0]=='-' && argv[i][1]=='I')
                ...
                else if(datastr=="Tzx") output=Tzx;
                else if(datastr=="Tzy") output=Tzy;
                else {
   ...
   case jones:
    ...
    for(int i=0;i<Tz.size();i++) {
        ...
        case Tzx:
                if(rotate!=0) {
                  if(i==0) rotate*=M_PI/180.;
                  cout<<setw(14)<<Tz[i].T
                      <<setw(14)<<Tz[i].x.real()*cos(rotate)+Tz[i].y.real()*-sin(rotate)
                      <<setw(14)<<Tz[i].x.imag()*cos(rotate)+Tz[i].y.imag()*-sin(rotate)
                      <<setw(14)<<sqrt(Tz[i].xvar*cos(rotate)*cos(rotate)+Tz[i].yvar*sin(rotate)*sin(rotate))<<endl;
                }
                else{
                  cout<<setw(14)<<Tz[i].T<<setw(14)<<Tz[i].x.real()<<setw(14)<<Tz[i].x.imag()<<setw(14)<<sqrt(Tz[i].xvar)<<endl;
                }
                break;
        case Tzy:
                if(rotate!=0) {
                  if(i==0) rotate*=M_PI/180.;
                  cout<<setw(14)<<Tz[i].T
                      <<setw(14)<<Tz[i].x.real()*sin(rotate)+Tz[i].y.real()*cos(rotate)
                      <<setw(14)<<Tz[i].x.imag()*sin(rotate)+Tz[i].y.imag()*cos(rotate)
                      <<setw(14)<<sqrt(Tz[i].xvar*sin(rotate)*sin(rotate)+Tz[i].yvar*cos(rotate)*cos(rotate))<<endl;
                }
                else{
                  cout<<setw(14)<<Tz[i].T<<setw(14)<<Tz[i].y.real()<<setw(14)<<Tz[i].y.imag()<<setw(14)<<sqrt(Tz[i].yvar)<<endl;
                }
                break;
        case Rzx:
