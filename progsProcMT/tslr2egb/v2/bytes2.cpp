...
bool correct_types(void) {
    if(sizeof(int2)==2 && sizeof(int4)==4 && sizeof(float8)==8)
        return true;
    else {
...
int8 bswap_64(int8 datum) {
    int4 lword= (datum & 0x00000000ffffffff);
    int4 hword=((datum & 0xffffffff00000000)>>32);
    lword=bswap_32(lword);
    hword=bswap_32(hword);
