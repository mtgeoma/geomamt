...
timecount startcount(string instr, table &tbl) {
    ...
    // starttime
    amxtds  STIM;
    in.read(reinterpret_cast <char *> (&STIM),sizeof(amxtds));
    ...
    in.read(reinterpret_cast <char *> (&sernum),2);
    if(bswap)
    ...
    unsigned short scan;
    in.read(reinterpret_cast <char *> (&scan),2);
    if(bswap)
        scan=bswap_16(scan);
    ...
    unsigned char chan;
    in.read(reinterpret_cast <char *> (&chan),1);
    if(chan!=5) {
    ...
    in.read(reinterpret_cast <char *> (&taglen),1);
    if(!taglen)
        taglen=16u;
    ...
    in.read(reinterpret_cast <char *> (&ETIM),sizeof(amxtds));
    if(ETIM.cen) {
