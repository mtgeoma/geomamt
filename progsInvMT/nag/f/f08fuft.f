...
C     .. Entry Points ..
      ENTRY             ZUNMTR(SIDE,UPLO,TRANS,M,N,A,LDA,TAU,C,LDC,WORK,
     *                  LWORK,INFO)
C
C  Purpose
C  =======
C
C  ZUNMTR overwrites the general complex m-by-n matrix C with
C
...
C  nq-1 elementary reflectors, as returned by ZHETRD:
C
...
C
     ...
     *         .AND. .NOT. (TRANS.EQ.'C' .OR. TRANS.EQ.'c')) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08FUF/ZUNMTR',-INFO)
         RETURN
      END IF
C
...
C
         CALL F08CUY(SIDE,TRANS,MI,NI,NQ-1,A(1,2),LDA,TAU,C,LDC,WORK,
     *               LWORK,IINFO)
      ELSE
C
