      SUBROUTINE F08DHZ(M,P,N,A,LDA,TAUA,B,LDB,TAUB,WORK,LWORK,INFO)
C     MARK 17 RELEASE. NAG COPYRIGHT 1995.
C
C  Purpose
C  =======
C
C  DGGRQF computes a generalized RQ factorization of an M-by-N matrix A
C  and a P-by-N matrix B:
C
C              A = R*Q,        B = Z*T*Q,
C
C  where Q is an N-by-N orthogonal matrix, Z is a P-by-P orthogonal
...
C          elements, with the array TAUA, represent the orthogonal
...
C
C  TAUA    (output) DOUBLE PRECISION array, dimension (min(M,N))
...
C          with the array TAUB, represent the orthogonal matrix Z as a
C          product of elementary reflectors (see Further Details).
C
C  LDB     (input) INTEGER
C          The leading dimension of the array B. LDB >= max(1,P).
C
C  TAUB    (output) DOUBLE PRECISION array, dimension (min(P,N))
C          The scalar factors of the elementary reflectors which
C          represent the orthogonal matrix Z (see Further Details).
C
...
C          blocksize for a call of DORMRQ.
C
...
C     H(i) = I - taua * v * v'
C
C  where taua is a real scalar, and v is a real vector with
C  v(n-k+i+1:n) = 0 and v(n-k+i) = 1; v(1:n-k+i-1) is stored on exit in
C  A(m-k+i,1:n-k+i-1), and taua in TAUA(i).
C  To form Q explicitly, use LAPACK subroutine DORGRQ.
C  To use Q to update another matrix, use LAPACK subroutine DORMRQ.
C
...
C     H(i) = I - taub * v * v'
C
C  where taub is a real scalar, and v is a real vector with
C  v(1:i-1) = 0 and v(i) = 1; v(i+1:p) is stored on exit in B(i+1:p,i),
C  and taub in TAUB(i).
C  To form Z explicitly, use LAPACK subroutine DORGQR.
C  To use Z to update another matrix, use LAPACK subroutine DORMQR.
C
...
C     .. External Subroutines ..
      EXTERNAL          DGEQRF, F06AAZ, F08CHZ, F08CKZ
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08DHZ',-INFO)
         RETURN
      END IF
C
C     RQ factorization of M-by-N matrix A: A = R*Q
C
      CALL F08CHZ(M,N,A,LDA,TAUA,WORK,LWORK,INFO)
      LOPT = WORK(1)
C
C     Update B := B*Q'
C
      CALL F08CKZ('Right','Transpose',P,N,MIN(M,N),A(MAX(1,M-N+1),1),
