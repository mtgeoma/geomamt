      SUBROUTINE F08CFZ(M,N,K,A,LDA,TAU,WORK,INFO)
C     MARK 16 RELEASE. NAG COPYRIGHT 1992.
C     ENTRY             DORG2L(M,N,K,A,LDA,TAU,WORK,INFO)
C
C  Purpose
C  =======
C
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08CFZ/DORG2L',-INFO)
         RETURN
      END IF
C
...
C
         A(M-N+II,II) = ONE
         CALL F08AEW('Left',M-N+II,II-1,A(1,II),1,TAU(I),A,LDA,WORK)
