      SUBROUTINE F08QHY(LTRANL,LTRANR,ISGN,N1,N2,TL,LDTL,TR,LDTR,B,LDB,
     *                  SCALE,X,LDX,XNORM,INFO)
C     MARK 16 RELEASE. NAG COPYRIGHT 1992.
C     ENTRY             DLASY2(LTRANL,LTRANR,ISGN,N1,N2,TL,LDTL,TR,
C    *                  LDTR,B,LDB,SCALE,X,LDX,XNORM,INFO)
C
C  Purpose
C  =======
C
...
C     .. External Functions ..
      ...
      EXTERNAL          X02AJF, X02AMF, IDAMAX, X02BHF
...
C
      ...
      END IF
      SCALE = ONE
      IF ((TWO*SMLNUM)*ABS(BTMP(2)).GT.ABS(U22) .OR. (TWO*SMLNUM)
     *    *ABS(BTMP(1)).GT.ABS(U11)) THEN
...
C
  ...
  200 CONTINUE
      ...
      SCALE = ONE
      IF ((EIGHT*SMLNUM)*ABS(BTMP(1)).GT.ABS(T16(1,1))
     *    .OR. (EIGHT*SMLNUM)*ABS(BTMP(2)).GT.ABS(T16(2,2))
     *    .OR. (EIGHT*SMLNUM)*ABS(BTMP(3)).GT.ABS(T16(3,3))
     *    .OR. (EIGHT*SMLNUM)*ABS(BTMP(4)).GT.ABS(T16(4,4))) THEN
