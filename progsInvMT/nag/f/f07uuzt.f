      SUBROUTINE F07UUZ(UPLO,TRANS,DIAG,NORMIN,N,AP,X,SCALE,CNORM,INFO)
C     MARK 15 RELEASE. NAG COPYRIGHT 1991.
C     MARK 16 REVISED. IER-1028 (JUN 1993).
C     ENTRY             ZLATPS(UPLO,TRANS,DIAG,NORMIN,N,AP,X,SCALE,
C    *                  CNORM,INFO)
C
C  Purpose
C  =======
C
C  ZLATPS solves one of the triangular systems
C
C     A * x = s*b,  A**T * x = s*b,  or  A**H * x = s*b,
C
C  with scaling to prevent overflow, where A is an upper or lower
C  triangular matrix stored in packed form.  Here A**T denotes the
C  transpose of A, A**H denotes the conjugate transpose of A, x and b
...
C  overflow, the Level 2 BLAS routine ZTPSV is called. If the matrix A
...
C  ======= =======
C
C  A rough bound on x is computed; if that is less than overflow, ZTPSV
...
C                                   1<=i< j
C
C  Since |x(j)| <= M(j), we use the Level 2 BLAS routine ZTPSV if the
...
C                      1<=i<=j
C
C  and we can safely call ZTPSV if 1/M(n) and 1/G(n) are both greater
...
C     .. External Subroutines ..
      EXTERNAL          DSCAL, F06AAZ, ZAXPY, ZDSCAL, ZTPSV
...
C
     ...
     *         .AND. .NOT. (NORMIN.EQ.'N' .OR. NORMIN.EQ.'n')) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07UUZ/ZLATPS',-INFO)
         RETURN
      END IF
C
...
C     Level 2 BLAS routine ZTPSV can be used.
C
      XMAX = ZERO
      DO 60 J = 1, N
