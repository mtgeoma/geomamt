...
C     .. Entry Points ..
      ENTRY             ZGBTRS(TRANS,N,KL,KU,NRHS,AB,LDAB,IPIV,B,LDB,
     *                  INFO)
C
C  Purpose
C  =======
C
C  ZGBTRS solves a system of linear equations
...
C
     ...
     *     .AND. .NOT. (TRANS.EQ.'C' .OR. TRANS.EQ.'c')) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07BSF/ZGBTRS',-INFO)
         RETURN
      END IF
C
            ...
            IF (LNOTI) THEN
               DO 20 J = 1, N - 1
                  ...
                  IF (L.NE.J) THEN
                     ...
                     B(J,I) = T
                  END IF
                  CALL ZAXPY(LM,-T,AB(KD+1,J),1,B(J+1,I),1)
   20          CONTINUE
            END IF
C
...
C
         DO 80 I = 1, NRHS
C
C           Solve U**T * X = B, overwriting B with X.
C
            CALL ZTBSV('Upper',TRANS,'Non-unit',N,KL+KU,AB,LDAB,B(1,I),
     *                 1)
C
C           Solve L**T * X = B, overwriting B with X.
C
            IF (LNOTI) THEN
               DO 60 J = N - 1, 1, -1
                  LM = MIN(KL,N-J)
                  B(J,I) = B(J,I) - ZDOTU(LM,AB(KD+1,J),1,B(J+1,I),1)
                  L = IPIV(J)
                  IF (L.NE.J) THEN
                     T = B(L,I)
                     B(L,I) = B(J,I)
...
C
            IF (LNOTI) THEN
               DO 100 J = N - 1, 1, -1
                  LM = MIN(KL,N-J)
                  B(J,I) = B(J,I) - ZDOTC(LM,AB(KD+1,J),1,B(J+1,I),1)
                  L = IPIV(J)
                  IF (L.NE.J) THEN
                     T = B(L,I)
                     B(L,I) = B(J,I)
