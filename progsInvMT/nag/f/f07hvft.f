...
C     .. Entry Points ..
      ENTRY             ZPBRFS(UPLO,N,KD,NRHS,AB,LDAB,AFB,LDAFB,B,LDB,X,
     *                  LDX,FERR,BERR,WORK,RWORK,INFO)
C
C  Purpose
C  =======
C
C  ZPBRFS improves the computed solution to a system of linear
...
C          F07HRF, in the same storage format as A (see AB).
C
...
C     .. External Subroutines ..
      EXTERNAL          ZAXPY, ZCOPY, ZHBMV, F04ZCF, F06AAZ, F07HSF
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07HVF/ZPBRFS',-INFO)
         RETURN
      END IF
C
...
C
            CALL F07HSF(UPLO,N,KD,1,AFB,LDAFB,WORK,N,INFO)
