...
C     .. Entry Points ..
      ENTRY             DPPTRI(UPLO,N,AP,INFO)
C
C  Purpose
C  =======
C
C  DPPTRI computes the inverse of a real symmetric positive definite
C  matrix A using the Cholesky factorization A = U'*U or A = L*L'
C  computed by F07GDF.
C
C  Arguments
C  =========
C
...
C     .. External Subroutines ..
      EXTERNAL          F06AAZ, F07UJF, DSCAL, DSPR, DTPMV
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07GJF/DPPTRI',-INFO)
         RETURN
      END IF
C
...
C
      CALL F07UJF(UPLO,'Non-unit',N,AP,INFO)
      IF (INFO.GT.0) RETURN
C
      IF (UPPER) THEN
C
...
C
         ...
         DO 40 J = 1, N
            ...
            IF (J.LT.N) CALL DTPMV('Lower','Transpose','Non-unit',N-J,
     *                             AP(JJN),AP(JJ+1),1)
            JJ = JJN
   40    CONTINUE
      END IF
