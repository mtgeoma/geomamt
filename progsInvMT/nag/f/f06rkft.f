...
C
         IF ((UPLO.EQ.'U' .OR. UPLO.EQ.'u')) THEN
            IF ((DIAG.EQ.'U' .OR. DIAG.EQ.'u')) THEN
               ...
               K = 2
               DO 560 J = 2, N
                  CALL F06FJF(J-1,AP(K),1,SCALE,SUM)
                  K = K + J
  560          CONTINUE
            ELSE
               SCALE = ZERO
               ...
               K = 1
               DO 580 J = 1, N
                  CALL F06FJF(J,AP(K),1,SCALE,SUM)
                  K = K + J
  580          CONTINUE
            END IF
         ELSE
            IF ((DIAG.EQ.'U' .OR. DIAG.EQ.'u')) THEN
               ...
               K = 2
               DO 600 J = 1, N - 1
                  CALL F06FJF(N-J,AP(K),1,SCALE,SUM)
                  K = K + N - J + 1
  600          CONTINUE
            ELSE
               SCALE = ZERO
               ...
               K = 1
               DO 620 J = 1, N
                  CALL F06FJF(N-J+1,AP(K),1,SCALE,SUM)
                  K = K + N - J + 1
  620          CONTINUE
            END IF
         END IF
