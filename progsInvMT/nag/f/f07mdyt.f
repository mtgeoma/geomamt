      SUBROUTINE F07MDY(UPLO,N,NB,KB,A,LDA,IPIV,W,LDW,INFO)
C     MARK 15 RELEASE. NAG COPYRIGHT 1991.
C     ENTRY             DLASYF(UPLO,N,NB,KB,A,LDA,IPIV,W,LDW,INFO)
C
C  Purpose
C  =======
C
...
C
C  DLASYF is an auxiliary routine called by F07MDF. It uses blocked code
...
C
C  W       (workspace) REAL array, dimension (LDW,NB)
C
C  LDW     (input) INTEGER
...
C
  ...
  120    CONTINUE
         ...
         END IF
         J = J + 1
         IF (JP.NE.JJ .AND. J.LE.N) CALL DSWAP(N-J+1,A(JP,J),LDA,
     *                                         A(JJ,J),LDA)
         IF (J.LE.N) GO TO 120
C
...
C
         ...
         CALL DGEMV('No transpose',N-K+1,K-1,-ONE,A(K,1),LDA,W(K,1),LDW,
     *              ONE,W(K,K),1)
C
         KSTEP = 1
C
...
C
            IF (J+JB.LE.N) CALL DGEMM('No transpose','Transpose',
...
C
  ...
  240    CONTINUE
         ...
         END IF
         J = J - 1
         IF (JP.NE.JJ) CALL DSWAP(J,A(JP,1),LDA,A(JJ,1),LDA)
         IF (J.GE.1) GO TO 240
C
C        Set KB to the number of columns factorized
C
