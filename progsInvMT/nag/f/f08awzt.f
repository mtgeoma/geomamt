      SUBROUTINE F08AWZ(M,N,K,A,LDA,TAU,WORK,INFO)
C     MARK 16 RELEASE. NAG COPYRIGHT 1992.
C     ENTRY             ZUNGL2(M,N,K,A,LDA,TAU,WORK,INFO)
C
C  Purpose
C  =======
C
C  ZUNGL2 generates an m-by-n complex matrix Q with orthonormal rows,
...
C     .. External Subroutines ..
      EXTERNAL          F06AAZ, F07FRY, F08ASW, ZSCAL
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08AWZ/ZUNGL2',-INFO)
         RETURN
      END IF
C
...
C
         IF (I.LT.N) THEN
            ...
            IF (I.LT.M) THEN
               A(I,I) = ONE
               CALL F08ASW('Right',M-I,N-I+1,A(I,I),LDA,DCONJG(TAU(I)),
     *                     A(I+1,I),LDA,WORK)
            END IF
