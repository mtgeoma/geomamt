...
C     .. Entry Points ..
      ENTRY             DTRTRS(UPLO,TRANS,DIAG,N,NRHS,A,LDA,B,LDB,INFO)
C
C  Purpose
C  =======
C
C  DTRTRS solves a real triangular system of linear equations with
C  multiple right hand sides, of the form
C
C     A * X = B  or  A' * X = B,
C
C  where A is a triangular matrix of order N, A' is the transpose of A,
...
C
     ...
     *         THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07TEF/DTRTRS',-INFO)
         RETURN
      END IF
C
...
C
      IF (NRHS.EQ.1) THEN
         CALL DTRSV(UPLO,TRANS,DIAG,N,A,LDA,B,1)
      ELSE
