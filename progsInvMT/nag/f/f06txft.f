...
C     ..
C
C  F06TXF  performs the transformation
C
C     A := P*A,            when   SIDE = 'L' or 'l'  (  Left-hand side )
C
C     A := A*conjg( P' ),  when   SIDE = 'R' or 'r'  ( Right-hand side )
C
C  where  A is an m by n matrix and P is a unitary matrix, consisting of
C  a sequence of plane rotations, applied in planes k1 to k2, determined
C  by the parameters PIVOT and DIRECT as follows:
C
               ...
               DO 220 J = K1, K2 - 1
                  IF( ( C( J ).NE.ONE ).OR.( S( J ).NE.ZERO ) )THEN
                     ...
                     DO 210 I = 1, M
                        ...
                        A( I, K2 ) = CTEMP*A( I, K2 ) - STEMPC*TEMP
  210                CONTINUE
                  END IF
  220          CONTINUE
            ELSE IF( ( DIRECT.EQ.'B' ).OR.( DIRECT.EQ.'b' ) )THEN
