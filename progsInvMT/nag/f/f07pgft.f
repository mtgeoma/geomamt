...
C     .. Entry Points ..
      ENTRY             DSPCON(UPLO,N,AP,IPIV,ANORM,RCOND,WORK,IWORK,
     *                  INFO)
C
C  Purpose
C  =======
C
C  DSPCON estimates the reciprocal of the condition number of a real
C  symmetric packed matrix A using the factorization A = U*D*U' or
C  A = L*D*L' computed by F07PDF.
C
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07PGF/DSPCON',-INFO)
         RETURN
      END IF
C
...
C
         CALL F07PEF(UPLO,N,1,AP,IPIV,WORK,N,INFO)
         GO TO 60
      END IF
C
