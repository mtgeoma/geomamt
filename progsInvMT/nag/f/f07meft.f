...
C     .. Entry Points ..
      ENTRY             DSYTRS(UPLO,N,NRHS,A,LDA,IPIV,B,LDB,INFO)
C
C  Purpose
C  =======
C
C  DSYTRS solves a system of linear equations A*X = B with a real
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07MEF/DSYTRS',-INFO)
         RETURN
      END IF
C
...
C
            CALL DGEMV('Transpose',K-1,NRHS,-ONE,B,LDB,A(1,K),1,ONE,
     *                 B(K,1),LDB)
            CALL DGEMV('Transpose',K-1,NRHS,-ONE,B,LDB,A(1,K+1),1,ONE,
     *                 B(K+1,1),LDB)
C
C           Interchange rows K and -IPIV(K).
C
...
C           stored in column K of A.
C
            IF (K.LT.N) CALL DGER(N-K,NRHS,-ONE,A(K+1,K),1,B(K,1),LDB,
     *                            B(K+1,1),LDB)
C
C           Multiply by the inverse of the diagonal block.
C
...
C
            IF (K.LT.N-1) THEN
               CALL DGER(N-K-1,NRHS,-ONE,A(K+2,K),1,B(K,1),LDB,B(K+2,1),
     *                   LDB)
