...
C     .. Entry Points ..
      ENTRY             DGEBAL(JOB,N,A,LDA,ILO,IHI,SCALE,INFO)
C
C  Purpose
C  =======
C
C  DGEBAL does permutations to isolate eigenvalues and/or balances a
C  real general matrix.  Balancing may reduce the 1-norm of the
...
C
     ...
     *    .AND. .NOT. (JOB.EQ.'B' .OR. JOB.EQ.'b')) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08NHF/DGEBAL',-INFO)
         RETURN
      END IF
C
      K = 1
