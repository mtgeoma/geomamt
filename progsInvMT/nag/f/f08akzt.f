      SUBROUTINE F08AKZ(SIDE,TRANS,M,N,K,A,LDA,TAU,C,LDC,WORK,INFO)
C     MARK 16 RELEASE. NAG COPYRIGHT 1992.
C     ENTRY             DORML2(SIDE,TRANS,M,N,K,A,LDA,TAU,C,LDC,WORK,
C    *                  INFO)
C
C  Purpose
C  =======
...
C
     ...
     *         (TRANS.EQ.'T' .OR. TRANS.EQ.'t')) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08AKZ/DORML2',-INFO)
         RETURN
      END IF
C
...
C
         AII = A(I,I)
         A(I,I) = ONE
         CALL F08AEW(SIDE,MI,NI,A(I,I),LDA,TAU(I),C(IC,JC),LDC,WORK)
         A(I,I) = AII
   20 CONTINUE
      RETURN
C
C     End of F08AKZ (DORML2)
C
      END
