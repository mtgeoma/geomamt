...
C     .. Entry Points ..
      ENTRY             ZBDSQR(UPLO,N,NCVT,NRU,NCC,D,E,VT,LDVT,U,LDU,C,
     *                  LDC,WORK,INFO)
C
C  Purpose
C  =======
C
C  ZBDSQR computes the singular value decomposition (SVD) of
...
C  or Q' * C, for given complex input matrices U, VT, and C.
C
...
C          as input.
C
C  VT      (input/output) COMPLEX*16 array, dimension (LDVT,NCVT)
...
C
C  U       (input/output) COMPLEX*16 array, dimension (LDU,N)
...
C
C  C       (input/output) COMPLEX*16 array, dimension (LDC,NCC)
...
C     .. External Subroutines ..
      EXTERNAL          F06AAZ, F06KPF, F06VXF, F08HEW, F08MEW, F08MEX,
     *                  F08MEY, ZDSCAL, ZSWAP
...
C
     ...
     *         .OR. (NCC.GT.0 .AND. LDC.LT.MAX(1,N))) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08MSF/ZBDSQR',-INFO)
         RETURN
      END IF
      IF (N.EQ.0) RETURN
      IF (N.EQ.1) GO TO 300
...
C
         IF (NRU.GT.0) CALL F06VXF('R','V','F',NRU,N,1,N,WORK(1),WORK(N)
     *                             ,U,LDU)
         IF (NCC.GT.0) CALL F06VXF('L','V','F',N,NCC,1,N,WORK(1),WORK(N)
     *                             ,C,LDC)
      END IF
C
...
C
         IF (NCVT.GT.0) CALL F06KPF(NCVT,VT(M-1,1),LDVT,VT(M,1),LDVT,
     *                              COSR,SINR)
         IF (NRU.GT.0) CALL F06KPF(NRU,U(1,M-1),1,U(1,M),1,COSL,SINL)
         IF (NCC.GT.0) CALL F06KPF(NCC,C(M-1,1),LDC,C(M,1),LDC,COSL,
     *                             SINL)
         M = M - 2
         GO TO 100
      END IF
...
C     Update singular vectors if desired
C
      IF (NCVT.GT.0) CALL F06VXF('L','V',DIRECT,M-LL+1,NCVT,1,M-LL+1,
     *                           WORK(NM12+LL),WORK(NM13+LL),VT(LL,1),
     *                           LDVT)
      IF (NRU.GT.0) CALL F06VXF('R','V',DIRECT,NRU,M-LL+1,1,M-LL+1,
     *                          WORK(LL),WORK(NM1+LL),U(1,LL),LDU)
      IF (NCC.GT.0) CALL F06VXF('L','V',DIRECT,M-LL+1,NCC,1,M-LL+1,
...
C
            D(ISUB) = D(N+1-I)
            D(N+1-I) = SMIN
            IF (NCVT.GT.0) CALL ZSWAP(NCVT,VT(ISUB,1),LDVT,VT(N+1-I,1),
     *                                LDVT)
            IF (NRU.GT.0) CALL ZSWAP(NRU,U(1,ISUB),1,U(1,N+1-I),1)
            IF (NCC.GT.0) CALL ZSWAP(NCC,C(ISUB,1),LDC,C(N+1-I,1),LDC)
         END IF
  360 CONTINUE
      GO TO 420
C
