...
C     .. Entry Points ..
      ENTRY             ZHPRFS(UPLO,N,NRHS,AP,AFP,IPIV,B,LDB,X,LDX,FERR,
     *                  BERR,WORK,RWORK,INFO)
C
C  Purpose
C  =======
C
C  ZHPRFS improves the computed solution to a system of linear
C  equations when the coefficient matrix is Hermitian indefinite
...
C          as computed by F07PRF, stored as a packed triangular matrix.
C
...
C     .. External Subroutines ..
      EXTERNAL          ZAXPY, ZCOPY, ZHPMV, F04ZCF, F06AAZ, F07PSF
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07PVF/ZHPRFS',-INFO)
         RETURN
      END IF
C
...
C
            CALL F07PSF(UPLO,N,1,AFP,IPIV,WORK,N,INFO)
