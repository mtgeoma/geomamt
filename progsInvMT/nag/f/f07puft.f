...
C     .. Entry Points ..
      ENTRY             ZHPCON(UPLO,N,AP,IPIV,ANORM,RCOND,WORK,INFO)
C
C  Purpose
C  =======
C
C  ZHPCON estimates the reciprocal of the condition number of a complex
C  Hermitian packed matrix A using the factorization A = U*D*U' or
C  A = L*D*L' computed by F07PRF.
C
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07PUF/ZHPCON',-INFO)
         RETURN
      END IF
C
...
C
         CALL F07PSF(UPLO,N,1,AP,IPIV,WORK,N,INFO)
         GO TO 60
      END IF
C
