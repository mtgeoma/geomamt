      SUBROUTINE F08CKZ(SIDE,TRANS,M,N,K,A,LDA,TAU,C,LDC,WORK,LWORK,
     *                  INFO)
...
C  =======
C
C  DORMRQ overwrites the general real M-by-N matrix C with
C
C                  SIDE = 'L'     SIDE = 'R'
C  TRANS = 'N':      Q * C          C * Q
...
C     .. External Subroutines ..
      EXTERNAL          F06AAZ, F07ZAZ, F08AEX, F08AEY, F08CKY
...
C
         CALL F08CKY(SIDE,TRANS,M,N,K,A,LDA,TAU,C,LDC,WORK,IINFO)
      ELSE
C
C        Use blocked code
C
...
C
            CALL F08AEX('Backward','Rowwise',NQ-K+I+IB-1,IB,A(I,1),LDA,
     *                  TAU(I),T,LDT)
            IF (LEFT) THEN
C
...
C           Apply H or H'
C
            CALL F08AEY(SIDE,TRANST,'Backward','Rowwise',MI,NI,IB,A(I,1)
