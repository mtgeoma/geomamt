...
C     .. Entry Points ..
      ENTRY             ZGBRFS(TRANS,N,KL,KU,NRHS,AB,LDAB,AFB,LDAFB,
     *                  IPIV,B,LDB,X,LDX,FERR,BERR,WORK,RWORK,INFO)
C
C  Purpose
C  =======
C
C  ZGBRFS improves the computed solution to a system of linear
...
C
C  AFB     (input) COMPLEX array, dimension (LDAFB,N)
...
C          The pivot indices from F07BRF; for 1<=i<=N, row i of the
...
C     .. External Subroutines ..
      EXTERNAL          ZAXPY, ZCOPY, ZGBMV, F04ZCF, F06AAZ, F07BSF
...
C
     ...
     *     .AND. .NOT. (TRANS.EQ.'C' .OR. TRANS.EQ.'c')) THEN
      ...
      ELSE IF (NRHS.LT.0) THEN
         INFO = -5
      ELSE IF (LDAB.LT.MAX(1,KL+KU+1)) THEN
         INFO = -7
      ELSE IF (LDAFB.LT.MAX(1,2*KL+KU+1)) THEN
         INFO = -9
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07BVF/ZGBRFS',-INFO)
         RETURN
      END IF
C
...
C
         ...
         CALL ZGBMV(TRANS,N,N,KL,KU,-ONE,AB,LDAB,X(1,J),1,ONE,WORK,1)
C
...
C
            CALL F07BSF(TRANS,N,KL,KU,1,AFB,LDAFB,IPIV,WORK,N,INFO)
...
C
               CALL F07BSF(TRANST,N,KL,KU,1,AFB,LDAFB,IPIV,WORK,N,INFO)
               DO 220 I = 1, N
