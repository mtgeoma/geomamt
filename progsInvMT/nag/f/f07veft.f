...
C     .. Entry Points ..
      ENTRY             DTBTRS(UPLO,TRANS,DIAG,N,KD,NRHS,AB,LDAB,B,LDB,
     *                  INFO)
C
C  Purpose
C  =======
C
C  DTBTRS solves a triangular system of the form
C
C     A * X = B  or  A' * X = B,
C
C  where A is a triangular band matrix of order N, A' is the transpose
C  of A, and B is an N by NRHS matrix.  A check is made to verify that A
C  is nonsingular.
C
C  Arguments
C  =========
...
C          = 'T':  Solve  A'* X = B  (Transpose)
C          = 'C':  Solve  A'* X = B  (Conjugate transpose = Transpose)
C
...
C
     ...
     *         THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07VEF/DTBTRS',-INFO)
         RETURN
      END IF
C
