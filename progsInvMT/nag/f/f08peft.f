...
C     .. Entry Points ..
      ENTRY             DHSEQR(JOB,COMPZ,N,ILO,IHI,H,LDH,WR,WI,Z,LDZ,
     *                  WORK,LWORK,INFO)
C
C  Purpose
C  =======
...
C  H = Z T Z', where T is an upper quasi-triangular matrix (the Schur
C  form), Z is the orthogonal matrix of Schur vectors, and Z' denotes
C  the transpose of Z.
C
...
C          This argument is redundant.
C
C  INFO    (output) INTEGER
C          = 0: successful exit
C          < 0: if INFO = -i, the i-th argument had an illegal value
C          > 0: DHSEQR failed to compute all the eigenvalues in a total
C               of 30*(IHI-ILO+1) iterations; if INFO = i, elements
C               1:ilo-1 and i+1:n of WR and WI contain those
...
C     .. External Functions ..
      DOUBLE PRECISION  F06BNF, F06RMF, X02AJF, X02AMF
      INTEGER           IDAMAX, X02BHF
      EXTERNAL          F06BNF, F06RMF, X02AJF, X02AMF, IDAMAX, X02BHF
C     .. External Subroutines ..
      EXTERNAL          DCOPY, DGEMV, DSCAL, F06AAZ, F06QFF, F06QHF,
     *                  F07ZAZ, F08AEV, F08PEW, F08PEZ
...
C
     ...
     *         .AND. .NOT. WANTZ) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08PEF/DHSEQR',-INFO)
         RETURN
      END IF
C
...
C
      CALL F07ZAZ(4,'F08PEF',NS,0)
      CALL F07ZAZ(8,'F08PEF',MAXB,0)
      IF (NS.LE.2 .OR. NS.GT.NH .OR. MAXB.GE.NH) THEN
C
C        Use the standard double-shift algorithm
C
         CALL F08PEZ(WANTT,WANTZ,N,ILO,IHI,H,LDH,WR,WI,ILO,IHI,Z,LDZ,
     *               INFO)
         GO TO 340
      END IF
      MAXB = MAX(3,MAXB)
...
C
         DO 120 K = I, L + 1, -1
            ...
            IF (ABS(H(K,K-1)).LE.MAX(ULP*TST1,SMLNUM)) GO TO 140
  120    CONTINUE
  140    CONTINUE
         L = K
         IF (L.GT.ILO) THEN
...
C
            CALL F06QFF('General',NS,NS,H(I-NS+1,I-NS+1),LDH,S,LDS)
            CALL F08PEZ(.FALSE.,.FALSE.,NS,1,NS,S,LDS,WR(I-NS+1),
     *                  WI(I-NS+1),1,NS,Z,LDZ,IERR)
            IF (IERR.GT.0) THEN
C
C              If F08PEZ failed to compute all NS eigenvalues, use the
...
C
                  ...
                  CALL DGEMV('No transpose',NV+1,NV,ONE,H(L,L),LDH,VV,1,
     *                       -WR(J),V,1)
                  NV = NV + 1
...
C
                  CALL DCOPY(NV+1,V,1,VV,1)
                  CALL DGEMV('No transpose',NV+1,NV,ONE,H(L,L),LDH,V,1,
     *                       -TWO*WR(J),VV,1)
                  ITEMP = IDAMAX(NV+1,VV,1)
                  TEMP = ONE/MAX(ABS(VV(ITEMP)),SMLNUM)
                  CALL DSCAL(NV+1,TEMP,VV,1)
                  ABSW = F06BNF(WR(J),WI(J))
                  TEMP = (TEMP*ABSW)*ABSW
                  CALL DGEMV('No transpose',NV+2,NV+1,ONE,H(L,L),LDH,VV,
     *                       1,TEMP,V,1)
                  NV = NV + 2
               END IF
C
...
C
            NR = MIN(NS+1,I-K+1)
            IF (K.GT.L) CALL DCOPY(NR,H(K,K-1),1,V,1)
            CALL F08AEV(NR,V(1),V(2),1,TAU)
            IF (K.GT.L) THEN
               H(K,K-1) = V(1)
               DO 260 II = K + 1, I
                  H(II,K-1) = ZERO
...
C           columns K to I2.
C
            CALL F08PEW('Left',NR,I2-K+1,V,TAU,H(K,K),LDH,WORK)
C
C           Apply G from the right to transform the columns of the
C           matrix in rows I1 to min(K+NR,I).
C
            CALL F08PEW('Right',MIN(K+NR,I)-I1+1,NR,V,TAU,H(I1,K),LDH,
     *                  WORK)
C
            IF (WANTZ) THEN
C
...
C
               CALL F08PEW('Right',NH,NR,V,TAU,Z(ILO,K),LDZ,WORK)
            END IF
  280    CONTINUE
C
  300 CONTINUE
...
C
      CALL F08PEZ(WANTT,WANTZ,N,L,I,H,LDH,WR,WI,ILO,IHI,Z,LDZ,INFO)
      IF (INFO.GT.0) RETURN
C
...
C
         DO 360 J = 1, N
            II = IDAMAX(N,Z(1,J),1)
            IF (Z(II,J).LT.ZERO) THEN
               CALL DSCAL(N,-ONE,Z(1,J),1)
               IF (J.GT.1) THEN
                  CALL DSCAL(J-1,-ONE,H(1,J),1)
                  H(J,J-1) = -H(J,J-1)
               END IF
               IF (J.LT.N) THEN
                  CALL DSCAL(N-J,-ONE,H(J,J+1),LDH)
                  H(J+1,J) = -H(J+1,J)
               END IF
            END IF
  360    CONTINUE
