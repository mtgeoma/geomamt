      SUBROUTINE F08CUY(SIDE,TRANS,M,N,K,A,LDA,TAU,C,LDC,WORK,LWORK,
     *                  INFO)
C     MARK 16 RELEASE. NAG COPYRIGHT 1992.
C     ENTRY             ZUNMQL(SIDE,TRANS,M,N,K,A,LDA,TAU,C,LDC,WORK,
C    *                  LWORK,INFO)
C
C  Purpose
C  =======
C
C  ZUNMQL overwrites the general complex m-by-n matrix C with
C
...
C        Q = H(k) . . . H(2) H(1)
C
C  as returned by ZGEQLF. Q is of order m if SIDE = 'L' and of order n
C  if SIDE = 'R'.
C
C  Arguments
C  =========
...
C          ZGEQLF in the last k columns of its array argument A.
...
C     .. External Subroutines ..
      EXTERNAL          F06AAZ, F07ZAZ, F08ASX, F08ASY, F08CUZ
...
C
     ...
     *         (TRANS.EQ.'C' .OR. TRANS.EQ.'c')) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08CUY/ZUNMQL',-INFO)
         RETURN
      END IF
C
...
C
         CALL F08CUZ(SIDE,TRANS,M,N,K,A,LDA,TAU,C,LDC,WORK,IINFO)
      ELSE
C
C        Use blocked code
C
...
C
            CALL F08ASX('Backward','Columnwise',NQ-K+I+IB-1,IB,A(1,I),
     *                  LDA,TAU(I),T,LDT)
            IF (LEFT) THEN
C
...
C           Apply H or H'
C
            CALL F08ASY(SIDE,TRANS,'Backward','Columnwise',MI,NI,IB,
     *                  A(1,I),LDA,T,LDT,C,LDC,WORK,LDWORK)
   20    CONTINUE
      END IF
      WORK(1) = IWS
      RETURN
C
C     End of F08CUY (ZUNMQL)
C
      END
