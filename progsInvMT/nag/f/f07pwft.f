...
C     .. Entry Points ..
      ENTRY             ZHPTRI(UPLO,N,AP,IPIV,WORK,INFO)
C
C  Purpose
C  =======
C
C  ZHPTRI computes the inverse of a complex Hermitian indefinite matrix
C  A in packed storage using the factorization A = U*D*U' or A = L*D*L'
C  computed by F07PRF.
C
C  Arguments
C  =========
C
...
C          used to obtain the factor U or L as computed by F07PRF,
...
C     .. External Subroutines ..
      EXTERNAL          ZCOPY, ZHPMV, ZSWAP, F06AAZ
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07PWF/ZHPTRI',-INFO)
         RETURN
      END IF
C
...
C
         KP = N*(N+1)/2
         DO 20 INFO = N, 1, -1
            IF (IPIV(INFO).GT.0 .AND. DBLE(AP(KP)).EQ.ZERO) RETURN
            KP = KP - INFO
   20    CONTINUE
      ELSE
C
...
C
            IF (K.GT.1) THEN
               ...
               AP(KC+K-1) = AP(KC+K-1) - DBLE(ZDOTC(K-1,WORK,1,AP(KC),1)
     *                      )
            END IF
C
...
C
            IF (K.GT.1) THEN
               ...
               AP(KC+K-1) = AP(KC+K-1) - DBLE(ZDOTC(K-1,WORK,1,AP(KC),1)
     *                      )
               AP(KCNEXT+K-1) = AP(KCNEXT+K-1) - ZDOTC(K-1,AP(KC),1,
     *                          AP(KCNEXT),1)
...
C
     ...
     *                    AP(KC+1),1)
               AP(KC) = AP(KC) - DBLE(ZDOTC(N-K,WORK,1,AP(KC+1),1))
               AP(KCNEXT+1) = AP(KCNEXT+1) - ZDOTC(N-K,AP(KC+1),1,
     *                        AP(KCNEXT+2),1)
               CALL ZCOPY(N-K,AP(KCNEXT+2),1,WORK,1)
               CALL ZHPMV(UPLO,N-K,-CONE,AP(KC+(N-K+1)),WORK,1,ZERO,
     *                    AP(KCNEXT+2),1)
               AP(KCNEXT) = AP(KCNEXT) - DBLE(ZDOTC(N-K,WORK,1,
     *                      AP(KCNEXT+2),1))
            END IF
C
