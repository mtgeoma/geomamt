...
C     .. Entry Points ..
      ENTRY             ZSYRFS(UPLO,N,NRHS,A,LDA,AF,LDAF,IPIV,B,LDB,X,
     *                  LDX,FERR,BERR,WORK,RWORK,INFO)
C
C  Purpose
C  =======
C
C  ZSYRFS improves the computed solution to a system of linear
...
C     .. External Subroutines ..
      EXTERNAL          ZAXPY, ZCOPY, F07NWZ, F04ZCF, F06AAZ, F07NSF
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07NVF/ZSYRFS',-INFO)
         RETURN
      END IF
C
...
C
         ...
         CALL F07NWZ(UPLO,N,-ONE,A,LDA,X(1,J),1,ONE,WORK,1)
C
...
C
            CALL F07NSF(UPLO,N,1,AF,LDAF,IPIV,WORK,N,INFO)
