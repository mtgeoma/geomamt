      SUBROUTINE F07ASG(TRANS,N,NRHS,A,LDA,PIV,B,LDB,INFO)
C     MARK 15 RELEASE. NAG COPYRIGHT 1991.
C     MARK 16 REVISED. IER-1020 (JUN 1993).
C
C  Purpose
C  =======
C
C  F07ASG solves a system of linear equations
...
C          as computed by F07ARG.
C
C  LDA     (input) INTEGER
C          The leading dimension of the array A.  LDA >= max(1,N).
C
C  PIV     (input) REAL array, dimension (N)
C          The pivot indices from F07ARG; for 1<=i<=N, row i of the
C          matrix was interchanged with row PIV(i).
C
...
C
C  This is a modified version of the LAPACK routine F07ASF/ZGETRS, in
C  which the INTEGER array IPIV has been replaced by a REAL array PIV,
C  and in the LU factorization U has unit diagonal elements.
C
C  =====================================================================
C
C     .. Parameters ..
...
C     .. External Subroutines ..
      EXTERNAL          F06AAZ, F07ARJ, ZTRSM, ZTRSV
...
C
         CALL F07ARJ(NRHS,B,LDB,1,N,PIV,1)
C
...
C
         IF (NRHS.EQ.1) THEN
            ...
            CALL ZTRSV('Upper','No transpose','Unit',N,A,LDA,B,1)
         ELSE
            CALL ZTRSM('Left','Lower','No transpose','Non-unit',N,NRHS,
     *                 ONE,A,LDA,B,LDB)
            CALL ZTRSM('Left','Upper','No transpose','Unit',N,NRHS,ONE,
     *                 A,LDA,B,LDB)
         END IF
      ELSE
C
...
C
         IF (NRHS.EQ.1) THEN
            CALL ZTRSV('Upper',TRANS,'Unit',N,A,LDA,B,1)
            CALL ZTRSV('Lower',TRANS,'Non-unit',N,A,LDA,B,1)
         ELSE
            CALL ZTRSM('Left','Upper',TRANS,'Unit',N,NRHS,ONE,A,LDA,B,
     *                 LDB)
            CALL ZTRSM('Left','Lower',TRANS,'Non-unit',N,NRHS,ONE,A,LDA,
     *                 B,LDB)
         END IF
C
