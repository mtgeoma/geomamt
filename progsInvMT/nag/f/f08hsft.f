...
C     .. Entry Points ..
      ENTRY             ZHBTRD(VECT,UPLO,N,KD,AB,LDAB,D,E,Q,LDQ,WORK,
     *                  INFO)
C
C  Purpose
C  =======
C
C  ZHBTRD reduces a complex Hermitian band matrix A to real symmetric
...
C
C  AB      (input/output) COMPLEX*16 array, dimension (LDAB,N)
...
C          If VECT = 'V', the n-by-n unitary matrix Q.
...
C     .. External Subroutines ..
      EXTERNAL          F06AAZ, F06THF, F07FRY, F07MRW, F08HSW, F08HSX,
     *                  F08HSY, F08HSZ, ZSCAL
...
C
     ...
     *         THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08HSF/ZHBTRD',-INFO)
         RETURN
      END IF
C
...
C
      IF (WANTQ) CALL F06THF('General',N,N,ZERO,ONE,Q,LDQ)
C
...
C
C     The real cosines and complex sines of the plane rotations are
C     stored in the arrays D and WORK.
C
      KDN = MIN(N-1,KD)
      IF (UPPER) THEN
C
         IF (KD.GT.1) THEN
C
C           Reduce to complex Hermitian tridiagonal form, working with
...
C
                     CALL F08HSZ(NR,AB(1,J1-1),KD1*LDAB,WORK(J1),KD1,
     *                           D(J1),KD1)
C
C                    apply rotations from the right
C
                     DO 20 L = 1, KD - 1
                        CALL F08HSY(NR,AB(L+1,J1-1),KD1*LDAB,AB(L,J1),
...
C                       within the band
C
                        CALL F08HSW(AB(KD-K+3,I+K-2),AB(KD-K+2,I+K-1),
     *                              D(I+K-1),WORK(I+K-1),TEMP)
                        AB(KD-K+3,I+K-2) = TEMP
C
C                       apply rotation from the right
C
                        CALL F07MRW(K-3,AB(KD-K+4,I+K-2),1,
...
C                 blocks
C
                  IF (NR.GT.0) CALL F08HSX(NR,AB(KD1,J1-1),AB(KD1,J1),
     *                                     AB(KD,J1),KD1*LDAB,D(J1),
     *                                     WORK(J1),KD1)
C
C                 apply plane rotations from the left
C
                  CALL F07FRY(NR,WORK(J1),KD1)
                  DO 40 L = 1, KD - 1
                     IF (J2+L.GT.N) THEN
                        NRT = NR - 1
                     ELSE
                        NRT = NR
                     END IF
                     IF (NRT.GT.0) CALL F08HSY(NRT,AB(KD-L,J1+L),
...
C
                     DO 60 J = J1, J2, KD1
                        CALL F07MRW(N,Q(1,J-1),1,Q(1,J),1,D(J),
     *                              DCONJG(WORK(J)))
   60                CONTINUE
                  END IF
C
...
C
         IF (KD.GT.0) THEN
C
C           make off-diagonal elements real and copy them to E
C
            DO 140 I = 1, N - 1
               T = AB(KD,I+1)
               ABST = ABS(T)
...
C                    elements which have been created outside the band
C
                     CALL F08HSZ(NR,AB(KD1,J1-KD1),KD1*LDAB,WORK(J1),
     *                           KD1,D(J1),KD1)
C
C                    apply plane rotations from one side
C
                     DO 200 L = 1, KD - 1
                        CALL F08HSY(NR,AB(KD1-L,J1-KD1+L),KD1*LDAB,
...
C                       within the band
C
                        CALL F08HSW(AB(K-1,I),AB(K,I),D(I+K-1),
     *                              WORK(I+K-1),TEMP)
                        AB(K-1,I) = TEMP
C
C                       apply rotation from the left
C
                        CALL F07MRW(K-3,AB(K-2,I+1),LDAB-1,AB(K-1,I+1),
...
C                 blocks
C
                  IF (NR.GT.0) CALL F08HSX(NR,AB(1,J1-1),AB(1,J1),
...
C
                  ...
                  DO 220 L = 1, KD - 1
                     ...
                     ELSE
                        NRT = NR
                     END IF
                     IF (NRT.GT.0) CALL F08HSY(NRT,AB(L+2,J1-1),
...
C
                     DO 240 J = J1, J2, KD1
                        CALL F07MRW(N,Q(1,J-1),1,Q(1,J),1,D(J),WORK(J))
  240                CONTINUE
                  END IF
C
                  IF (J2+KDN.GT.N) THEN
