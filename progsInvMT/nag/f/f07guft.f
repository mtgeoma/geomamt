...
C     .. Entry Points ..
      ENTRY             ZPPCON(UPLO,N,AP,ANORM,RCOND,WORK,RWORK,INFO)
C
C  Purpose
C  =======
C
C  ZPPCON estimates the reciprocal of the condition number of a complex
C  Hermitian positive definite packed matrix using the Cholesky
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07GUF/ZPPCON',-INFO)
         RETURN
      END IF
C
...
C           Multiply by inv(U').
C
            CALL F07UUZ('Upper','Conjugate transpose','Non-unit',NORMIN,
     *                  N,AP,WORK,SCALEL,RWORK,INFO)
            NORMIN = 'Y'
C
C           Multiply by inv(U).
C
            CALL F07UUZ('Upper','No transpose','Non-unit',NORMIN,N,AP,
     *                  WORK,SCALEU,RWORK,INFO)
         ELSE
C
C           Multiply by inv(L).
C
            CALL F07UUZ('Lower','No transpose','Non-unit',NORMIN,N,AP,
     *                  WORK,SCALEL,RWORK,INFO)
            NORMIN = 'Y'
C
C           Multiply by inv(L').
C
            CALL F07UUZ('Lower','Conjugate transpose','Non-unit',NORMIN,
