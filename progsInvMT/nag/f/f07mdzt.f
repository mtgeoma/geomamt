      SUBROUTINE F07MDZ(UPLO,N,A,LDA,IPIV,INFO)
C     MARK 15 RELEASE. NAG COPYRIGHT 1991.
C     ENTRY             DSYTF2(UPLO,N,A,LDA,IPIV,INFO)
C
C  Purpose
C  =======
C
...
C     .. External Subroutines ..
      EXTERNAL          F06AAZ, F07MDX, DROT, DSCAL, DSWAP, DSYR
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07MDZ/DSYTF2',-INFO)
         RETURN
      END IF
C
...
C
               CALL DSWAP(KP,A(1,KK),1,A(1,KP),1)
               DO 40 J = KK, KP, -1
                  T = A(J,KK)
                  A(J,KK) = A(KP,J)
                  A(KP,J) = T
...
C
               CALL F07MDX(A(K-1,K-1),A(K-1,K),A(K,K),R1,R2,C,S)
               R1 = ONE/R1
               R2 = ONE/R2
...
C
               CALL DSWAP(N-KP+1,A(KP,KK),1,A(KP,KP),1)
               DO 80 J = KK, KP
                  T = A(J,KK)
                  A(J,KK) = A(KP,J)
                  A(KP,J) = T
...
C
                  R1 = ONE/A(K,K)
                  CALL DSYR(UPLO,N-K,-R1,A(K+1,K),1,A(K+1,K+1),LDA)
C
...
C                 decomposition of D(k)
C
                  CALL F07MDX(A(K,K),A(K+1,K),A(K+1,K+1),R1,R2,C,S)
                  R1 = ONE/R1
                  R2 = ONE/R2
                  CALL DROT(N-K-1,A(K+2,K),1,A(K+2,K+1),1,C,S)
                  CALL DSYR(UPLO,N-K-1,-R1,A(K+2,K),1,A(K+2,K+2),LDA)
                  CALL DSYR(UPLO,N-K-1,-R2,A(K+2,K+1),1,A(K+2,K+2),LDA)
C
