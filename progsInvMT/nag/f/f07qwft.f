...
C     .. Entry Points ..
      ENTRY             ZSPTRI(UPLO,N,AP,IPIV,WORK,INFO)
C
C  Purpose
C  =======
C
C  ZSPTRI computes the inverse of a complex symmetric indefinite matrix
C  A in packed storage using the factorization A = U*D*U' or A = L*D*L'
C  computed by F07QRF.
C
C  Arguments
C  =========
C
...
C          used to obtain the factor U or L as computed by F07QRF,
...
C     .. External Subroutines ..
      EXTERNAL          ZCOPY, F07QWZ, ZSWAP, F06AAZ
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07QWF/ZSPTRI',-INFO)
         RETURN
      END IF
C
...
C
            IF (K.GT.1) THEN
               ...
               CALL F07QWZ(UPLO,K-1,-ONE,AP,WORK,1,ZERO,AP(KC),1)
...
C
            IF (K.GT.1) THEN
               ...
               CALL F07QWZ(UPLO,K-1,-ONE,AP,WORK,1,ZERO,AP(KC),1)
               AP(KC+K-1) = AP(KC+K-1) - ZDOTU(K-1,WORK,1,AP(KC),1)
               AP(KCNEXT+K-1) = AP(KCNEXT+K-1) - ZDOTU(K-1,AP(KC),1,
     *                          AP(KCNEXT),1)
               CALL ZCOPY(K-1,AP(KCNEXT),1,WORK,1)
               CALL F07QWZ(UPLO,K-1,-ONE,AP,WORK,1,ZERO,AP(KCNEXT),1)
               AP(KCNEXT+K) = AP(KCNEXT+K) - ZDOTU(K-1,WORK,1,AP(KCNEXT)
     *                        ,1)
            END IF
C
C           Interchange rows and columns K and -IPIV(K).
...
C
            IF (K.LT.N) THEN
               ...
               CALL F07QWZ(UPLO,N-K,-ONE,AP(KC+N-K+1),WORK,1,ZERO,
     *                    AP(KC+1),1)
...
C
     ...
     *                    AP(KC+1),1)
               AP(KC) = AP(KC) - ZDOTU(N-K,WORK,1,AP(KC+1),1)
               AP(KCNEXT+1) = AP(KCNEXT+1) - ZDOTU(N-K,AP(KC+1),1,
     *                        AP(KCNEXT+2),1)
               CALL ZCOPY(N-K,AP(KCNEXT+2),1,WORK,1)
               CALL F07QWZ(UPLO,N-K,-ONE,AP(KC+(N-K+1)),WORK,1,ZERO,
     *                    AP(KCNEXT+2),1)
               AP(KCNEXT) = AP(KCNEXT) - ZDOTU(N-K,WORK,1,AP(KCNEXT+2),
     *                      1)
            END IF
C
