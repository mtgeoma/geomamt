...
C     ..
C
C  F06TYF  performs the transformation
C
...
C
C     R( k ) = ( conjg( c( k ) )  s( k ) ),   with  s( k )  real.
C              (       -s( k )    c( k ) )
C
            ...
            IF( ( DIRECT.EQ.'F' ).OR.( DIRECT.EQ.'f' ) )THEN
               DO 180 J = K1 + 1, K2
                  ...
                  IF( ( CTEMP.NE.ONE ).OR.( STEMP.NE.ZERO ) )THEN
                     ...
                     DO 170 I = 1, M
                        TEMP = A( I, J )
                        A( I, J ) = CTEMPC*TEMP - STEMP*A( I, K1 )
               ...
               DO 220 J = K1, K2 - 1
                  IF( ( C( J ).NE.ONE ).OR.( S( J ).NE.ZERO ) )THEN
                     ...
                     DO 210 I = 1, M
                        ...
                        A( I, K2 ) = CTEMPC*A( I, K2 ) - STEMP*TEMP
  210                CONTINUE
                  END IF
  220          CONTINUE
            ELSE IF( ( DIRECT.EQ.'B' ).OR.( DIRECT.EQ.'b' ) )THEN
