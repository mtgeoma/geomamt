...
C     .. Entry Points ..
      ENTRY             DPPTRF(UPLO,N,AP,INFO)
C
C  Purpose
C  =======
C
C  DPPTRF computes the Cholesky factorization of a real symmetric
C  positive definite matrix A stored in packed format.
C
...
C  when N = 4, UPLO = 'U':
C
C  Two-dimensional storage of the symmetric matrix A:
C
C     a11 a12 a13 a14
C         a22 a23 a24
C             a33 a34     (aij = aji)
...
C     .. External Subroutines ..
      EXTERNAL          F06AAZ, DSCAL, DSPR, DTPSV
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07GDF/DPPTRF',-INFO)
         RETURN
      END IF
C
...
C
            IF (J.GT.1) CALL DTPSV('Upper','Transpose','Non-unit',J-1,
     *                             AP,AP(JC),1)
C
