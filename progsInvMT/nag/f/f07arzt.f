      SUBROUTINE F07ARZ(M,N,A,LDA,IPIV,INFO)
C     MARK 15 RELEASE. NAG COPYRIGHT 1991.
C     MARK 16 REVISED. IER-1018 (JUN 1993).
C     ENTRY             ZGETF2(M,N,A,LDA,IPIV,INFO)
C
C  Purpose
C  =======
C
C  ZGETF2 computes an LU factorization of a general m-by-n matrix A
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07ARZ/ZGETF2',-INFO)
         RETURN
      END IF
C
...
C
            IF (J.LT.M) CALL ZSCAL(M-J,ONE/A(J,J),A(J+1,J),1)
C
...
C
            CALL ZGERU(M-J,N-J,-ONE,A(J+1,J),1,A(J,J+1),LDA,A(J+1,J+1),
     *                 LDA)
         END IF
   20 CONTINUE
      RETURN
C
C     End of F07ARZ (ZGETF2)
C
      END
