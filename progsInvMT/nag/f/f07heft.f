...
C     .. Entry Points ..
      ENTRY             DPBTRS(UPLO,N,KD,NRHS,AB,LDAB,B,LDB,INFO)
C
C  Purpose
C  =======
C
C  DPBTRS solves a system of linear equations A*X = B with a symmetric
C  positive definite band matrix A using the Cholesky factorization
C  A = U'*U or A = L*L' computed by F07HDF.
C
C  Arguments
C  =========
C
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07HEF/DPBTRS',-INFO)
         RETURN
      END IF
C
