...
C     .. Entry Points ..
      ENTRY             DORMQR(SIDE,TRANS,M,N,K,A,LDA,TAU,C,LDC,WORK,
     *                  LWORK,INFO)
C
C  Purpose
C  =======
...
C     .. External Subroutines ..
      EXTERNAL          F06AAZ, F07ZAZ, F08AEX, F08AEY, F08AGZ
...
C
     ...
     *         (TRANS.EQ.'T' .OR. TRANS.EQ.'t')) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08AGF/DORMQR',-INFO)
         RETURN
      END IF
C
...
C
         CALL F08AGZ(SIDE,TRANS,M,N,K,A,LDA,TAU,C,LDC,WORK,IINFO)
      ELSE
C
C        Use blocked code
C
...
C
            CALL F08AEX('Forward','Columnwise',NQ-I+1,IB,A(I,I),LDA,
     *                  TAU(I),T,LDT)
            IF (LEFT) THEN
C
...
C           Apply H or H'
C
            CALL F08AEY(SIDE,TRANS,'Forward','Columnwise',MI,NI,IB,
