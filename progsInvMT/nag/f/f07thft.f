...
C     .. Entry Points ..
      ENTRY             DTRRFS(UPLO,TRANS,DIAG,N,NRHS,A,LDA,B,LDB,X,LDX,
     *                  FERR,BERR,WORK,IWORK,INFO)
C
C  Purpose
C  =======
C
C  DTRRFS provides error bounds and backward error estimates for the
C  solution to a system of linear equations with a triangular
C  coefficient matrix.
C
C  The solution vectors X must be computed by F07TEF or some other
C  means before entering this routine.  DTRRFS does not do iterative
...
C     .. External Subroutines ..
      EXTERNAL          F04YCF, F06AAZ, DAXPY, DCOPY, DTRMV, DTRSV
...
C
     ...
     *         THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07THF/DTRRFS',-INFO)
         RETURN
      END IF
C
