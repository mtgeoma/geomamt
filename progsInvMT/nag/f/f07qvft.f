...
C     .. Entry Points ..
      ENTRY             ZSPRFS(UPLO,N,NRHS,AP,AFP,IPIV,B,LDB,X,LDX,FERR,
     *                  BERR,WORK,RWORK,INFO)
C
C  Purpose
C  =======
C
C  ZSPRFS improves the computed solution to a system of linear
C  equations when the coefficient matrix is symmetric indefinite
...
C          The factored form of the matrix A.  AFP contains the block
C          diagonal matrix D and the multipliers used to obtain the
C          factor U or L from the factorization A = U*D*U' or A = L*D*L'
C          as computed by F07QRF, stored as a packed triangular matrix.
C
...
C     .. External Subroutines ..
      EXTERNAL          ZAXPY, ZCOPY, F04ZCF, F06AAZ, F07QSF, F07QWZ
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07QVF/ZSPRFS',-INFO)
         RETURN
      END IF
C
...
C
         ...
         CALL F07QWZ(UPLO,N,-ONE,AP,X(1,J),1,ONE,WORK,1)
C
         ...
         IF (UPPER) THEN
            DO 100 K = 1, N
               ...
               DO 80 I = 1, K - 1
                  ...
                  IK = IK + 1
   80          CONTINUE
               RWORK(K) = RWORK(K) + CABS1(AP(IK))*XK + S
               KK = KK + K
  100       CONTINUE
         ELSE
            DO 140 K = 1, N
...
C
            CALL F07QSF(UPLO,N,1,AFP,IPIV,WORK,N,INFO)
