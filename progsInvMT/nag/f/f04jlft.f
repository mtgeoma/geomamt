...
C  =======
C
C  DGGGLM solves a general Gauss-Markov linear model (GLM) problem:
C
...
C
C  X       (output) DOUBLE PRECISION array, dimension (M)
C  Y       (output) DOUBLE PRECISION array, dimension (P)
...
C     .. Parameters ..
      ...
      CHARACTER*6       SRNAME
      PARAMETER         (SRNAME='F04JLF')
C     .. Scalar Arguments ..
      INTEGER           IFAIL, LDA, LDB, LWORK, M, N, P
C     .. Array Arguments ..
      DOUBLE PRECISION  A(LDA,*), B(LDB,*), D(*), WORK(LWORK), X(*),
     *                  Y(*)
C     .. Local Scalars ..
...
C     .. External Subroutines ..
      EXTERNAL          DCOPY, DGEMV, DORMQR, DTRSV, F08CKZ, F08DEZ
...
C
         NP = MIN(N,P)
         CALL F08DEZ(N,M,P,A,LDA,WORK,B,LDB,WORK(M+1),WORK(M+NP+1),
     *               LWORK-M-NP,INFO)
         LOPT = WORK(M+NP+1)
C
...
C
         IF (N.GT.M) CALL DGEMV('No transpose',M,N-M,-ONE,B(1,M+P-N+1),
     *                          LDB,Y(M+P-N+1),1,ONE,D,1)
C
...
C
         IF (P.GT.0) THEN
            CALL F08CKZ('Left','Transpose',P,1,NP,B(MAX(1,N-P+1),1),LDB,
     *                  WORK(M+1),Y,MAX(1,P),WORK(M+NP+1),LWORK-M-NP,
     *                  INFO)
