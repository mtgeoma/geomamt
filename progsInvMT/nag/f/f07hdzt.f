      SUBROUTINE F07HDZ(UPLO,N,KD,AB,LDAB,INFO)
C     MARK 15 RELEASE. NAG COPYRIGHT 1991.
C     ENTRY             DPBTF2(UPLO,N,KD,AB,LDAB,INFO)
C
C  Purpose
C  =======
C
C  DPBTF2 computes the Cholesky factorization of a real symmetric
C  positive definite band matrix A.
C
C  The factorization has the form
C     A = U' * U ,  if UPLO = 'U', or
C     A = L  * L',  if UPLO = 'L',
C  where U is an upper triangular matrix, U' is the transpose of U, and
...
C     .. External Subroutines ..
      EXTERNAL          F06AAZ, DSCAL, DSYR, DTRSV
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07HDZ/DPBTF2',-INFO)
         RETURN
      END IF
C
...
C
            KN = MIN(J-1,KD)
            CALL DTRSV('Upper','Transpose','Non-unit',KN,AB(KD+1,J-KN),
     *                 KLD,AB(KD+1-KN,J),1)
C
C           Compute U(J,J) and test for non-positive-definiteness.
C
            AJJ = AB(KD+1,J) - DDOT(KN,AB(KD+1-KN,J),1,AB(KD+1-KN,J),1)
            IF (AJJ.LE.ZERO) THEN
               AB(KD+1,J) = AJJ
               GO TO 60
            END IF
