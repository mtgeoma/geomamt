      SUBROUTINE F08AXZ(SIDE,TRANS,M,N,K,A,LDA,TAU,C,LDC,WORK,INFO)
C     MARK 16 RELEASE. NAG COPYRIGHT 1992.
C     ENTRY             ZUNML2(SIDE,TRANS,M,N,K,A,LDA,TAU,C,LDC,WORK,
C    *                  INFO)
C
C  Purpose
C  =======
C
C  ZUNML2 overwrites the general complex m-by-n matrix C with
C
...
C
     ...
     *         (TRANS.EQ.'C' .OR. TRANS.EQ.'c')) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08AXZ/ZUNML2',-INFO)
         RETURN
      END IF
C
...
C
         ...
         ELSE
            TAUI = TAU(I)
         END IF
         IF (I.LT.NQ) CALL F07FRY(NQ-I,A(I,I+1),LDA)
         AII = A(I,I)
         A(I,I) = ONE
         CALL F08ASW(SIDE,MI,NI,A(I,I),LDA,TAUI,C(IC,JC),LDC,WORK)
         A(I,I) = AII
         IF (I.LT.NQ) CALL F07FRY(NQ-I,A(I,I+1),LDA)
   20 CONTINUE
      RETURN
C
C     End of F08AXZ (ZUNML2)
C
      END
