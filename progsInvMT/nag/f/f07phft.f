...
C     .. Entry Points ..
      ENTRY             DSPRFS(UPLO,N,NRHS,AP,AFP,IPIV,B,LDB,X,LDX,FERR,
     *                  BERR,WORK,IWORK,INFO)
C
C  Purpose
C  =======
C
C  DSPRFS improves the computed solution to a system of linear
...
C          as computed by F07PDF, stored as a packed triangular matrix.
C
...
C     .. External Subroutines ..
      EXTERNAL          F04YCF, F06AAZ, F07PEF, DAXPY, DCOPY, DSPMV
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07PHF/DSPRFS',-INFO)
         RETURN
      END IF
C
...
C
            CALL F07PEF(UPLO,N,1,AFP,IPIV,WORK(N+1),N,INFO)
