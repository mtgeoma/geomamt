...
C     ..
C
C  F06TMF performs the transformation
C
C     A := P*A*conjg( P' ),
C
C  where  A  is an  n by n hermitian matrix and  P  is a unitary matrix,
...
C     part of  A must contain the upper triangular part of the hermitian
C     matrix  and the  strictly  lower  triangular  part  of  A  is  not
C     referenced.
C
C     When  UPLO = 'L' or 'l'  then the leading  n by n lower triangular
C     part of  A must contain the lower triangular part of the hermitian
C     matrix  and the  strictly  upper  triangular  part  of  A  is  not
C     referenced.
C
C  The imaginary parts of the diagonal elements of the array  A need not
C  be set and are assumed to be  zero. On return, they are set to  zero.
C
...
C     .. Local Scalars ..
      ...
      COMPLEX*16         AIJ, STEMP, STEMPC, TEMP
C     .. Parameters ..
...
C
C              First use  F06CHF  to perform the  two by two  similarity
C              transformations  to those  elements  that get  changed by
C              P( j ), j = k1, ..., k2 - 1,  both from the  left and the
C              right.  See the comment in  F06CHF  for an explanation of
...
C
               DO 20 J = K1, K2 - 1
                  IF( ( C( J ).NE.ONE ).OR.( S( J ).NE.ZERO ) )THEN
                     ...
                     STEMP = S( J )
                     STEMPC = DCONJG( STEMP )
                     CALL F06CHF( A( J + 1, J + 1 ), A( J + 1, J ),
     $                            A( J, J ), CTEMP, -STEMPC )
                     DO 10 I = J + 2, N
                        TEMP = A( I, J + 1 )
                        A( I, J + 1 ) = CTEMP*TEMP - STEMPC*A( I, J )
...
C
               DO 60 J = K2 - 2, 1, -1
                  ...
                  DO 50 I = K2 - 1, I2, -1
                     ...
                     AIJ = DCONJG( S( I ) )*AIJ + C( I )*TEMP
   50             CONTINUE
                  A( I2, J ) = AIJ
   60          CONTINUE
C
C              Next use  F06CHF  to perform  the  two by two  similarity
C              transformations  to those  elements  that get  changed by
...
C
                     DO 100 I = J + 1, N
                        TEMP = A( I, J )
                        A( I, J ) = CTEMP*TEMP - STEMPC*A( I, K1 )
...
C
                  ...
                  IF( ( CTEMP.NE.ONE ).OR.( STEMP.NE.ZERO ) )THEN
                     STEMPC = DCONJG( STEMP )
C
C                    Next  use   F06CHF  to  perform  the   two  by  two
C                    similarity transformation  to those  elements  that
C                    get changed  both  from  the  left  and the  right.
C
                     CALL F06CHF( A( J, J ), A( J, K1 ), A( K1, K1 ),
...
C           is stored in the  lower triangle of  A,  some rotations have
C           to be  performed  rowwise.  This  may be  disadvantageous on
C           paged machines.
C
            IF( ( DIRECT.EQ.'F' ).OR.( DIRECT.EQ.'f' ) )THEN
               DO 230 J = K1, K2 - 1
...
C
                  ...
                  DO 210 I = K1, J - 1
                     AIJ = DCONJG( A( J, I ) )
                     A( J, I ) = DCONJG( C( I )*AIJ + DCONJG( S( I ) )*
     $                           TEMP )
...
C                    get  changed  both  from  the  left  and the right.
C
                     CALL F06CHF( A( K2, K2 ), A( K2, J ), A( J, J ),
     $                            CTEMP, -STEMPC )
C
C                    Next  apply the  right hand rotations,  columnwise.
C
                     DO 220 I = K2 + 1, N
                        ...
                        A( I, K2 ) = -STEMPC*TEMP + CTEMP*A( I, K2 )
  220                CONTINUE
                  END IF
  230          CONTINUE
C
...
C
               DO 320 J = K2 - 2, K1, -1
                  IF( ( C( J ).NE.ONE ).OR.( S( J ).NE.ZERO ) )THEN
                     ...
                     DO 310 I = K2 - 1, J + 1, -1
                        TEMP = DCONJG( A( K2, I ) )
                        A( K2, I ) = DCONJG( -STEMPC*A( I, J ) + CTEMP*
     $                               TEMP )
...
C                 columnwise, and use  F06CHF  to perform the two by two
...
C
  ...
  430                CONTINUE
C
                     CALL F06CHF( A( K1, K1 ), A( K1, J ), A( J, J ),
     $                            CTEMP, STEMP )
C
                  END IF
  440          CONTINUE
...
C
               DO 480 J = K1 + 2, K2
                  ...
                  IF( ( CTEMP.NE.ONE ).OR.( STEMP.NE.ZERO ) )THEN
                     ...
                     DO 470 I = K1 + 1, J - 1
                        TEMP = DCONJG( A( K1, I ) )
                        A( K1, I ) = DCONJG( CTEMP*TEMP + STEMP*
     $                               A( I, J ) )
                        A( I, J ) = -STEMPC*TEMP + CTEMP*A( I, J )
  470                CONTINUE
                  END IF
  480          CONTINUE
            ELSE IF( ( DIRECT.EQ.'B' ).OR.( DIRECT.EQ.'b' ) )THEN
...
C
                     DO 500 I = 1, K1 - 1
                        TEMP = A( I, J )
                        A( I, J ) = -STEMPC*A( I, K1 ) + CTEMP*TEMP
...
C                    get  changed  both  from  the  left  and the right.
C
                     CALL F06CHF( A( J, J ), A( J, K2 ), A( K2, K2 ),
     $                            CTEMP, STEMP )
C
C                    Next  apply the  right hand rotations,  columnwise.
C
