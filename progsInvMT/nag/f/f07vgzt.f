      SUBROUTINE F07VGZ(UPLO,TRANS,DIAG,NORMIN,N,KD,AB,LDAB,X,SCALE,
     *                  CNORM,INFO)
C     MARK 15 RELEASE. NAG COPYRIGHT 1991.
C     MARK 16 REVISED. IER-1029 (JUN 1993).
C     ENTRY             DLATBS(UPLO,TRANS,DIAG,NORMIN,N,KD,AB,LDAB,X,
C    *                  SCALE,CNORM,INFO)
C
C  Purpose
C  =======
C
C  DLATBS solves one of the triangular systems
C
...
C  overflow, the Level 2 BLAS routine DTBSV is called.  If the matrix A
...
C  ======= =======
C
C  A rough bound on x is computed; if that is less than overflow, DTBSV
...
C                                   1<=i< j
C
C  Since |x(j)| <= M(j), we use the Level 2 BLAS routine DTBSV if the
...
C                      1<=i<=j
C
C  and we can safely call DTBSV if 1/M(n) and 1/G(n) are both greater
...
C     .. External Subroutines ..
      EXTERNAL          DAXPY, DSCAL, DTBSV, F06AAZ
...
C
     ...
     *         .AND. .NOT. (NORMIN.EQ.'N' .OR. NORMIN.EQ.'n')) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07VGZ/DLATBS',-INFO)
         RETURN
      END IF
C
...
C     Level 2 BLAS routine DTBSV can be used.
C
      J = IDAMAX(N,X,1)
      XMAX = ABS(X(J))
      XBND = XMAX
...
C
                  JLEN = MIN(KD,N-J)
                  IF (JLEN.GT.0) CALL DAXPY(JLEN,-X(J)*TSCAL,AB(2,J),1,
     *                                      X(J+1),1)
