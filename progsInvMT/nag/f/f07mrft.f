...
C     .. Entry Points ..
      ENTRY             ZHETRF(UPLO,N,A,LDA,IPIV,WORK,LWORK,INFO)
C
C  Purpose
C  =======
C
C  ZHETRF computes the factorization of a complex Hermitian matrix A
...
C     .. External Subroutines ..
      EXTERNAL          F06AAZ, F07MRY, F07MRZ, F07ZAZ
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07MRF/ZHETRF',-INFO)
         RETURN
      END IF
C
C     Determine the block size
C
      CALL F07ZAZ(1,'F07MRF',NB,0)
      IF (NB.LE.1) NB = N
C
      IF (NB.LT.N) THEN
         LDWORK = N
...
C        KB, where KB is the number of columns factorized by F07MRY;
...
C
            CALL F07MRY(UPLO,K,NB,KB,A,LDA,IPIV,WORK,LDWORK,IINFO)
         ELSE
C
C           Use unblocked code to factorize columns 1:k of A
C
            CALL F07MRZ(UPLO,K,A,LDA,IPIV,IINFO)
            KB = K
         END IF
C
...
C
            CALL F07MRY(UPLO,N-K+1,NB,KB,A(K,K),LDA,IPIV(K),WORK,LDWORK,
     *                  IINFO)
         ELSE
C
C           Use unblocked code to factorize columns k:n of A
C
            CALL F07MRZ(UPLO,N-K+1,A(K,K),LDA,IPIV(K),IINFO)
            KB = N - K + 1
         END IF
C
C        Set INFO on the first occurrence of a zero pivot
