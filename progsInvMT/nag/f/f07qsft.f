...
C     .. Entry Points ..
      ENTRY             ZSPTRS(UPLO,N,NRHS,AP,IPIV,B,LDB,INFO)
C
C  Purpose
C  =======
C
C  ZSPTRS solves a system of linear equations A*X = B with a complex
C  symmetric matrix A stored in packed format using the factorization
C  A = U*D*U' or A = L*D*L' computed by F07QRF.
C
C  Arguments
C  =========
C
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07QSF/ZSPTRS',-INFO)
         RETURN
      END IF
C
...
C
            CALL ZGEMV('Transpose',K-1,NRHS,-ONE,B,LDB,AP(KC),1,ONE,
     *                 B(K,1),LDB)
            CALL ZGEMV('Transpose',K-1,NRHS,-ONE,B,LDB,AP(KC+K),1,ONE,
     *                 B(K+1,1),LDB)
C
C           Interchange rows K and -IPIV(K).
C
