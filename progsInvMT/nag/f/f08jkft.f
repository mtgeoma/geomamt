...
C     MARK 17 REVISED. IER-1653 (JUN 1995).
C     .. Entry Points ..
      ENTRY             DSTEIN(N,D,E,M,W,IBLOCK,ISPLIT,Z,LDZ,WORK,IWORK,
     *                  IFAIL,INFO)
C
C  Purpose
C  =======
C
C  DSTEIN computes the eigenvectors of a real symmetric tridiagonal
C  matrix T corresponding to specified eigenvalues, using inverse
C  iteration.
C
...
C     .. External Functions ..
      DOUBLE PRECISION  DASUM, DDOT, DNRM2, X02AJF
      INTEGER           IDAMAX, X02BHF
      EXTERNAL          DASUM, DDOT, DNRM2, X02AJF, IDAMAX, X02BHF
C     .. External Subroutines ..
      EXTERNAL          DAXPY, DCOPY, DSCAL, F01LEF, F04LEF, F06AAZ,
     *                  G05FAF
...
C
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08JKF/DSTEIN',-INFO)
         RETURN
      END IF
C
...
C           close enough.
C
            IF (JBLK.EQ.1) GO TO 180
            IF (ABS(XJ-XJM).GT.ORTOL) GPIND = J
            DO 160 I = GPIND, J - 1
               CTR = -DDOT(BLKSIZ,WORK(INDRV1+1),1,Z(B1,I),1)
               CALL DAXPY(BLKSIZ,CTR,Z(B1,I),1,WORK(INDRV1+1),1)
  160       CONTINUE
C
