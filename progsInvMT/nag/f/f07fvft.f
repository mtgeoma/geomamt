...
C     .. Entry Points ..
      ENTRY             ZPORFS(UPLO,N,NRHS,A,LDA,AF,LDAF,B,LDB,X,LDX,
     *                  FERR,BERR,WORK,RWORK,INFO)
C
C  Purpose
C  =======
C
C  ZPORFS improves the computed solution to a system of linear
...
C     .. External Subroutines ..
      EXTERNAL          ZAXPY, ZCOPY, ZHEMV, F04ZCF, F06AAZ, F07FSF
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07FVF/ZPORFS',-INFO)
         RETURN
      END IF
C
...
C
            CALL F07FSF(UPLO,N,1,AF,LDAF,WORK,N,INFO)
