...
C     .. Entry Points ..
      ENTRY             DGEBAK(JOB,SIDE,N,ILO,IHI,SCALE,M,V,LDV,INFO)
C
C  Purpose
C  =======
C
C  DGEBAK forms the right or left eigenvectors of a real general matrix
C  by backward transformation on the computed eigenvectors of the matrix
C  preprocessed by DGEBAL.
C
C  Arguments
C  =========
C
...
C                 balancing.
C          JOB must be the same as the JOB parameter in DGEBAL.
C
...
C  ILO     (input) INTEGER
C  IHI     (input) INTEGER
C          ILO and IHI are integers determined by DGEBAL.
C
C  SCALE   (input) DOUBLE PRECISION array, dimension (N)
C          SCALE contains information determining the permutations
C          and/or scaling factors used by DGEBAL.
C
C  M       (input) INTEGER
C          M is the number of columns of the matrix of eigenvectors V
C          to be back transformed.
C
...
C
     ...
     *    .AND. .NOT. (JOB.EQ.'B' .OR. JOB.EQ.'b')) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08NJF/DGEBAK',-INFO)
         RETURN
      END IF
C
