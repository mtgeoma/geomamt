...
C     .. Entry Points ..
      ENTRY             DORGTR(UPLO,N,A,LDA,TAU,WORK,LWORK,INFO)
C
C  Purpose
C  =======
C
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08FFF/DORGTR',-INFO)
         RETURN
      END IF
C
...
C
         CALL F08CFY(N-1,N-1,N-1,A,LDA,TAU,WORK,LWORK,IINFO)
C
      ELSE
C
