      SUBROUTINE F08NSY(N,K,NB,A,LDA,TAU,T,LDT,Y,LDY)
C     MARK 16 RELEASE. NAG COPYRIGHT 1992.
C     ENTRY             ZLAHRD(N,K,NB,A,LDA,TAU,T,LDT,Y,LDY)
C
C  Purpose
C  =======
C
C  ZLAHRD reduces the first NB columns of a complex general n-by-(n-k+1)
C  matrix A so that elements below the k-th subdiagonal are zero. The
C  reduction is performed by a unitary similarity transformation
...
C
C  This is an auxiliary routine called by ZGEHRD.
C
C  Arguments
C  =========
C
...
C
C  A       (input/output) COMPLEX*16 array, dimension (LDA,N-K+1)
...
C
C  TAU     (output) COMPLEX*16 array, dimension (NB)
C          The scalar factors of the elementary reflectors. See Further
C          Details.
C
C  T       (output) COMPLEX*16 array, dimension (NB,NB)
...
C     .. External Subroutines ..
      EXTERNAL          F07FRY, F08ASV, ZAXPY, ZCOPY, ZGEMV, ZSCAL,
     *                  ZTRMV
...
C
         EI = A(K+I,I)
         CALL F08ASV(N-K-I+1,EI,A(MIN(K+I+1,N),I),1,TAU(I))
         A(K+I,I) = ONE
C
C        Compute  Y(1:n,i)
C
