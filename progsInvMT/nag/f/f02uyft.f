      SUBROUTINE F02UYF(N,D,E,NCOLB,B,LDB,NROWY,Y,LDY,NCOLZ,Z,LDZ,WORK,
     *                  IFAIL)
C     MARK 14 RELEASE. NAG COPYRIGHT 1989.
C  1. Purpose
C     =======
C
C  F02UYF  reduces  an  n by n  real bidiagonal matrix  to diagonal form
C  by  means  of  orthogonal transformations.  The  transformations  may
C  optionally be applied to given complex matrices.
C
C  2. Description
C     ===========
C
C  The n by n real bidiagonal matrix A is factorized as
C
C     A = Q*S*P',
C
C  where  Q and P are n by n orthogonal matrices and S is an n by n real
...
C
C  Optionally  the  complex matrices  C  and/or  Y  and/or  Z  given  by
C
C     C = Q'*B,   W = Y*Q,   X = P'*Z,
C
C  where  B is an n by ncolb complex matrix,  Y is an nrowy by n complex
C  matrix and  Z is an  n by ncolz complex matrix, can also be returned.
C
...
C
C           E  is  used  as  internal  workspace  by  F02UYF  and so the
...
C           Unchanged on exit.
C
C  Y      - COMPLEX array of DIMENSION ( LDY, n ).
C
...
C           Unchanged on exit.
C
C  Z      - COMPLEX array of DIMENSION ( LDZ, ncolz ).
C
...
C
C           The array  WORK  is used as  internal workspace  by  F02UYF.
...
C     ===================
C
C  This routine  can be  used in conjunction  with routines  F02UWF  and
C  F02UXF   to  find  the  singular  value  decomposition  of  an  upper
C  triangular matrix.
C
C
...
C        split and  p .gt. 0  means that  e( p )  is negligible, or
C        will be negligible after forcing the split.
C
...
C
               ...
               IF (WANTZ) CALL F06VXF('Left','Bottom','Backward',N,
...
C
               ...
               IF (WANTB) CALL F06VXF('Left','Top','Forward',N,NCOLB,P,
     *                                K,WORK,WORK(IW1),B,LDB)
               IF (WANTY) CALL F06VXF('Right','Top','Forward',NROWY,N,P,
...
C
     ...
     *                  WORK,WORK(IW1),WANTZ,WORK(IW2),WORK(IW3))
            IF (WANTB) CALL F06VXF('Left','Variable','Forward',N,NCOLB,
     *                             L,K,WORK,WORK(IW1),B,LDB)
            IF (WANTY) CALL F06VXF('Right','Variable','Forward',NROWY,N,
     *                             L,K,WORK,WORK(IW1),Y,LDY)
            IF (WANTZ) CALL F06VXF('Left','Variable','Forward',N,NCOLZ,
...
C
      DO 60 I = K, N
         IF (D(I).LT.ZERO) THEN
            D(I) = -D(I)
            IF (WANTB) CALL F06HGF(NCOLB,B(I,1),LDB)
            IF (WANTY) CALL F06HGF(NROWY,Y(1,I),1)
         END IF
   60 CONTINUE
C
...
C
  ...
  100    CONTINUE
         ...
         END IF
  120 CONTINUE
      IF (WANTB) CALL F06VKF('Left','Transpose',N,WORK,NCOLB,B,LDB)
      IF (WANTY) CALL F06VKF('Right','No Transpose',N,WORK,NROWY,Y,LDY)
      IF (WANTZ) CALL F06VKF('Left','Transpose',N,WORK,NCOLZ,Z,LDZ)
      WORK(1) = ITER
      IF (K.EQ.1) THEN
         IFAIL = P01ABF(IFAIL,0,SRNAME,0,REC)
      ELSE
