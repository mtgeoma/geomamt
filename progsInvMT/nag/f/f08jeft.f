...
C     MARK 17 REVISED. IER-1651 (JUN 1995).
C     .. Entry Points ..
      ENTRY             DSTEQR(COMPZ,N,D,E,Z,LDZ,WORK,INFO)
C
C  Purpose
C  =======
C
...
C
C  DSTEQR can also compute the eigenvectors of a real symmetric matrix
C  A, which has been reduced to tridiagonal form by DSYTRD, DSPTRD or
C  DSBTRD.
C
C  Arguments
C  =========
...
C          On entry, if COMPZ = 'V', Z must contain the orthogonal
...
C               represent a tridiagonal matrix which is orthogonally
...
C     .. External Subroutines ..
      EXTERNAL          DROT, DSCAL, DSWAP, F06AAZ, F06QHF, F06QXF,
     *                  F07MDX, F08HEW, F08JEX
...
C
     ...
     *    THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08JEF/DSTEQR',-INFO)
         RETURN
      END IF
C
...
C
      IF (INITZ) CALL F06QHF('General',N,N,ZERO,ONE,Z,LDZ)
C
      ITN = N*MAXIT
      MM = 0
C
...
C
               CALL F06QXF('R','V','B',N,M-L+1,1,M-L+1,WORK(L),
...
C              Apply saved rotations.
C
               CALL F06QXF('R','V','F',N,M-L+1,1,M-L+1,WORK(L),
     *                     WORK(N-1+L),Z(1,L),LDZ)
            END IF
         ELSE
            IF (L.EQ.M-1) THEN
...
C
               IF (WANTZ) THEN
                  ...
                  CALL DROT(N,Z(1,M-1),1,Z(1,M),1,C,S)
               ELSE
...
C
  ...
  300    CONTINUE
         IF (K.NE.I) THEN
            D(K) = D(I)
            D(I) = P
            IF (WANTZ) CALL DSWAP(N,Z(1,I),1,Z(1,K),1)
         END IF
  320 CONTINUE
C
      IF (WANTZ) THEN
