...
C     .. Entry Points ..
      ENTRY             ZHPTRF(UPLO,N,AP,IPIV,INFO)
C
C  Purpose
C  =======
C
C  ZHPTRF computes the factorization of a complex Hermitian packed
C  matrix A using the Bunch-Kaufman diagonal pivoting method:
C
...
C
C  A       (input/output) COMPLEX array, dimension (N*(N+1)/2)
...
C     .. External Subroutines ..
      EXTERNAL          ZHPR, ZDSCAL, ZSWAP, F06AAZ, F07MRW, F07MRX
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07PRF/ZHPTRF',-INFO)
         RETURN
      END IF
C
...
C
                  KP = K
               ELSE IF (ABS(DBLE(AP(KPC+IMAX-1))).GE.ALPHA*ROWMAX) THEN
C
...
C
               CALL F07MRX(AP(KC-1),AP(KC+K-2),AP(KC+K-1),R1,R2,C,S)
               R1 = ONE/R1
               R2 = ONE/R2
               CALL F07MRW(K-2,AP(KNC),1,AP(KC),1,C,S)
...
C                 decomposition of D(k)
C
                  CALL F07MRX(AP(KC),DCONJG(AP(KC+1)),AP(KNC),R1,R2,C,S)
                  R1 = ONE/R1
                  R2 = ONE/R2
                  CALL F07MRW(N-K-1,AP(KC+2),1,AP(KNC+1),1,C,S)
                  CALL ZHPR(UPLO,N-K-1,-R1,AP(KC+2),1,AP(KNC+N-K))
                  CALL ZHPR(UPLO,N-K-1,-R2,AP(KNC+1),1,AP(KNC+N-K))
C
