...
C     .. Entry Points ..
      ENTRY             ZSYCON(UPLO,N,A,LDA,IPIV,ANORM,RCOND,WORK,INFO)
C
C  Purpose
C  =======
C
C  ZSYCON estimates the reciprocal of the condition number of a complex
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07NUF/ZSYCON',-INFO)
         RETURN
      END IF
C
...
C
         CALL F07NSF(UPLO,N,1,A,LDA,IPIV,WORK,N,INFO)
         GO TO 60
      END IF
C
