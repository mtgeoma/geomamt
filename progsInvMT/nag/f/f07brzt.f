      SUBROUTINE F07BRZ(M,N,KL,KU,AB,LDAB,IPIV,INFO)
C     MARK 15 RELEASE. NAG COPYRIGHT 1991.
C     ENTRY             ZGBTF2(M,N,KL,KU,AB,LDAB,IPIV,INFO)
C
C  Purpose
C  =======
C
C  ZGBTF2 computes an LU factorization of a complex m-by-n band matrix
C  A using partial pivoting with row interchanges.
C
...
C     .. External Subroutines ..
      EXTERNAL          ZGERU, ZSCAL, ZSWAP, F06AAZ
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07BRZ/ZGBTF2',-INFO)
         RETURN
      END IF
C
...
C
            IF (JP.NE.1) CALL ZSWAP(JU-J+1,AB(KV+JP,J),LDAB-1,AB(KV+1,J)
     *                              ,LDAB-1)
            IF (KM.GT.0) THEN
C
...
C
               IF (JU.GT.J) CALL ZGERU(KM,JU-J,-ONE,AB(KV+2,J),1,
