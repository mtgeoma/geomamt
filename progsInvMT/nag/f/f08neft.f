...
C     .. Entry Points ..
      ENTRY             DGEHRD(N,ILO,IHI,A,LDA,TAU,WORK,LWORK,INFO)
C
C  Purpose
C  =======
C
...
C     .. External Subroutines ..
      EXTERNAL          DGEMM, F06AAZ, F07ZAZ, F08AEY, F08NEY, F08NEZ
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08NEF/DGEHRD',-INFO)
         RETURN
      END IF
C
...
C
            CALL F08NEY(IHI,I,IB,A(1,I),LDA,TAU(I),T,LDT,WORK,LDWORK)
C
...
C           left
C
            CALL F08AEY('Left','Transpose','Forward','Columnwise',IHI-I,
     *                  N-I-IB+1,IB,A(I+1,I),LDA,T,LDT,A(I+1,I+IB),LDA,
     *                  WORK,LDWORK)
   60    CONTINUE
      END IF
C
...
C
      CALL F08NEZ(N,I,IHI,A,LDA,TAU,WORK,IINFO)
      WORK(1) = IWS
C
      RETURN
C
C     End of F08NEF (DGEHRD)
C
      END
