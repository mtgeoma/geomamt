...
C     .. Entry Points ..
      ENTRY             ZHECON(UPLO,N,A,LDA,IPIV,ANORM,RCOND,WORK,INFO)
C
C  Purpose
C  =======
C
C  ZHECON estimates the reciprocal of the condition number of a complex
C  Hermtiain matrix A using the factorization A = U*D*U' or A = L*D*L'
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07MUF/ZHECON',-INFO)
         RETURN
      END IF
C
...
C
         CALL F07MSF(UPLO,N,1,A,LDA,IPIV,WORK,N,INFO)
         GO TO 60
      END IF
C
