      SUBROUTINE F08CUZ(SIDE,TRANS,M,N,K,A,LDA,TAU,C,LDC,WORK,INFO)
C     MARK 16 RELEASE. NAG COPYRIGHT 1992.
C     ENTRY             ZUNM2L(SIDE,TRANS,M,N,K,A,LDA,TAU,C,LDC,WORK,
C    *                  INFO)
C
C  Purpose
C  =======
C
C  ZUNM2L overwrites the general complex m-by-n matrix C with
C
...
C
     ...
     *         (TRANS.EQ.'C' .OR. TRANS.EQ.'c')) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08CUZ/ZUNM2L',-INFO)
         RETURN
      END IF
C
...
C
         ...
         AII = A(NQ-K+I,I)
         A(NQ-K+I,I) = ONE
         CALL F08ASW(SIDE,MI,NI,A(1,I),1,TAUI,C,LDC,WORK)
         A(NQ-K+I,I) = AII
   20 CONTINUE
      RETURN
C
C     End of F08CUZ (ZUNM2L)
C
      END
