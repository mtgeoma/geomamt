      SUBROUTINE F08DEZ(N,M,P,A,LDA,TAUA,B,LDB,TAUB,WORK,LWORK,INFO)
C     MARK 17 RELEASE. NAG COPYRIGHT 1995.
C
C  Purpose
C  =======
C
C  DGGQRF computes a generalized QR factorization of an N-by-M matrix A
...
C          with the array TAUA, represent the orthogonal matrix Q as a
...
C
C  TAUA    (output) DOUBLE PRECISION array, dimension (min(N,M))
...
C          elements, with the array TAUB, represent the orthogonal
...
C
C  TAUB    (output) DOUBLE PRECISION array, dimension (min(N,P))
C          The scalar factors of the elementary reflectors which
...
C          blocksize for a call of DORMQR.
C
...
C  To form Q explicitly, use LAPACK subroutine DORGQR.
C  To use Q to update another matrix, use LAPACK subroutine DORMQR.
C
...
C  To form Z explicitly, use LAPACK subroutine DORGRQ.
C  To use Z to update another matrix, use LAPACK subroutine DORMRQ.
C
...
C     .. External Subroutines ..
      EXTERNAL          DGEQRF, DORMQR, F06AAZ, F08CHZ
...
C
      CALL F08CHZ(N,P,B,LDB,TAUB,WORK,LWORK,INFO)
      WORK(1) = MAX(LOPT,INT(WORK(1)))
C
      RETURN
C
C     End of F08DEZ (DGGQRF)
C
      END
