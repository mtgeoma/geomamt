...
C     .. Entry Points ..
      ENTRY             ZUPMTR(SIDE,UPLO,TRANS,M,N,AP,TAU,C,LDC,WORK,
     *                  INFO)
C
C  Purpose
C  =======
C
C  ZUPMTR overwrites the general complex m-by-n matrix C with
C
...
C  nq-1 elementary reflectors, as returned by ZHPTRD using packed
C  storage:
C
...
C
C  AP      (input) COMPLEX*16 array, dimension
C                               (M*(M+1)/2) if SIDE = 'L'
C                               (N*(N+1)/2) if SIDE = 'R'
C          The vectors which define the elementary reflectors, as
C          returned by ZHPTRD.
C
C  TAU     (input) COMPLEX*16 array, dimension (M-1) if SIDE = 'L'
C                                     or (N-1) if SIDE = 'R'
...
C
     ...
     *         (TRANS.EQ.'C' .OR. TRANS.EQ.'c')) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08GUF/ZUPMTR',-INFO)
         RETURN
      END IF
C
...
C
         DO 20 I = I1, I2, I3
            IF (LEFT) THEN
C
C              H(i) or H(i)' is applied to C(1:i,1:n)
C
               MI = I
            ELSE
C
C              H(i) or H(i)' is applied to C(1:m,1:i)
C
               NI = I
            END IF
C
...
C
            ...
            AII = AP(II)
            AP(II) = ONE
            CALL F08ASW(SIDE,MI,NI,AP(II-I+1),1,TAUI,C,LDC,WORK)
            AP(II) = AII
C
            IF (FORWRD) THEN
               II = II + I + 2
...
C
         DO 40 I = I1, I2, I3
            ...
            IF (LEFT) THEN
C
C              H(i) or H(i)' is applied to C(i+1:m,1:n)
C
               MI = M - I
               IC = I + 1
            ELSE
C
C              H(i) or H(i)' is applied to C(1:m,i+1:n)
C
               NI = N - I
               JC = I + 1
            END IF
...
C
            ...
            ELSE
               TAUI = DCONJG(TAU(I))
            END IF
            CALL F08ASW(SIDE,MI,NI,AP(II),1,TAUI,C(IC,JC),LDC,WORK)
            AP(II) = AII
C
            IF (FORWRD) THEN
               II = II + NQ - I + 1
