...
C     .. Entry Points ..
      ENTRY             DGERFS(TRANS,N,NRHS,A,LDA,AF,LDAF,IPIV,B,LDB,X,
     *                  LDX,FERR,BERR,WORK,IWORK,INFO)
C
C  Purpose
C  =======
C
C  DGERFS improves the computed solution to a system of linear
...
C     .. External Subroutines ..
      EXTERNAL          F04YCF, F06AAZ, F07AEF, DAXPY, DCOPY, DGEMV
...
C
     ...
     *     .AND. .NOT. (TRANS.EQ.'C' .OR. TRANS.EQ.'c')) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07AHF/DGERFS',-INFO)
         RETURN
      END IF
C
...
C
            CALL F07AEF(TRANS,N,1,AF,LDAF,IPIV,WORK(N+1),N,INFO)
...
C
               CALL F07AEF(TRANST,N,1,AF,LDAF,IPIV,WORK(N+1),N,INFO)
               DO 220 I = 1, N
