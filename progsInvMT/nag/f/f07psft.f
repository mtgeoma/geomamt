...
C     .. Entry Points ..
      ENTRY             ZHPTRS(UPLO,N,NRHS,AP,IPIV,B,LDB,INFO)
C
C  Purpose
C  =======
C
C  ZHPTRS solves a system of linear equations A*X = B with a complex
C  Hermitian matrix A stored in packed format using the factorization
C  A = U*D*U' or A = L*D*L' computed by F07PRF.
C
C  Arguments
C  =========
C
...
C          obtain the factor U or L as computed by F07PRF, stored as a
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07PSF/ZHPTRS',-INFO)
         RETURN
      END IF
C
...
C
            ...
            CALL ZGERU(K-2,NRHS,-ONE,AP(KC-(K-1)),1,B(K-1,1),LDB,B(1,1),
     *                 LDB)
C
C           Multiply by the inverse of the diagonal block.
C
...
C           stored in column K of A.
C
            IF (K.LT.N) CALL ZGERU(N-K,NRHS,-ONE,AP(KC+1),1,B(K,1),LDB,
     *                             B(K+1,1),LDB)
C
C           Multiply by the inverse of the diagonal block.
C
...
C
            IF (K.LT.N-1) THEN
               CALL ZGERU(N-K-1,NRHS,-ONE,AP(KC+2),1,B(K,1),LDB,B(K+2,1)
     *                    ,LDB)
               CALL ZGERU(N-K-1,NRHS,-ONE,AP(KC+N-K+2),1,B(K+1,1),LDB,
     *                    B(K+2,1),LDB)
            END IF
C
C           Multiply by the inverse of the diagonal block.
