      SUBROUTINE F07ARJ(N,A,LDA,K1,K2,PIV,INCX)
C     MARK 15 RELEASE. NAG COPYRIGHT 1991.
C
C  Purpose
C  =======
C
C  F07ARJ performs a series of row interchanges on the matrix A.
...
C  F07ARY/ZLASWP, in which the INTEGER array IPIV has been replaced by
C  a REAL array PIV.
C
C     .. Scalar Arguments ..
...
C
      ...
      IF (INCX.EQ.1) THEN
         DO 20 I = K1, K2
            IP = NINT(PIV(I))
            IF (IP.NE.I) CALL ZSWAP(N,A(I,1),LDA,A(IP,1),LDA)
   20    CONTINUE
