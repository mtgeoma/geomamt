...
C     .. Entry Points ..
      ENTRY             ZTPRFS(UPLO,TRANS,DIAG,N,NRHS,AP,B,LDB,X,LDX,
     *                  FERR,BERR,WORK,RWORK,INFO)
C
C  Purpose
C  =======
C
C  ZTPRFS provides error bounds and backward error estimates for the
C  solution to a system of linear equations with a triangular packed
C  coefficient matrix.
C
C  The solution vectors X must be computed by F07USF or some other
C  means before entering this routine.  ZTPRFS does not do iterative
...
C     .. External Subroutines ..
      EXTERNAL          ZAXPY, ZCOPY, ZTPMV, ZTPSV, F04ZCF, F06AAZ
...
C
     ...
     *         THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07UVF/ZTPRFS',-INFO)
         RETURN
      END IF
C
