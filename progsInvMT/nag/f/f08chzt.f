      SUBROUTINE F08CHZ(M,N,A,LDA,TAU,WORK,LWORK,INFO)
C     MARK 17 RELEASE. NAG COPYRIGHT 1995.
C
C  Purpose
C  =======
C
C  DGERQF computes an RQ factorization of a real M-by-N matrix A:
C  A = R * Q.
C
C  Arguments
C  =========
...
C     .. External Subroutines ..
      EXTERNAL          F06AAZ, F07ZAZ, F08AEX, F08AEY, F08CHY
...
C
            CALL F08CHY(IB,N-K+I+IB-1,A(M-K+I,1),LDA,TAU(I),WORK,IINFO)
            IF (M-K+I.GT.1) THEN
C
C              Form the triangular factor of the block reflector
C              H = H(i+ib-1) . . . H(i+1) H(i)
C
               CALL F08AEX('Backward','Rowwise',N-K+I+IB-1,IB,A(M-K+I,1)
     *                     ,LDA,TAU(I),WORK,LDWORK)
C
C              Apply H to A(1:m-k+i-1,1:n-k+i+ib-1) from the right
C
               CALL F08AEY('Right','No transpose','Backward','Rowwise',
...
C
      IF (MU.GT.0 .AND. NU.GT.0) CALL F08CHY(MU,NU,A,LDA,TAU,WORK,IINFO)
C
      WORK(1) = IWS
      RETURN
C
C     End of F08CHZ (DGERQF)
C
      END
