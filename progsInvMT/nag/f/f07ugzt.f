      SUBROUTINE F07UGZ(UPLO,TRANS,DIAG,NORMIN,N,AP,X,SCALE,CNORM,INFO)
C     MARK 15 RELEASE. NAG COPYRIGHT 1991.
C     MARK 16 REVISED. IER-1027 (JUN 1993).
C     ENTRY             DLATPS(UPLO,TRANS,DIAG,NORMIN,N,AP,X,SCALE,
C    *                  CNORM,INFO)
C
C  Purpose
C  =======
C
C  DLATPS solves one of the triangular systems
C
C     A *x = s*b  or  A'*x = s*b
C
C  with scaling to prevent overflow, where A is an upper or lower
C  triangular matrix stored in packed form.  Here A' denotes the
C  transpose of A, x and b are n-element vectors, and s is a scaling
C  factor, usually less than or equal to 1, chosen so that the
...
C  DTPSV is called. If the matrix A is singular (A(j,j) = 0 for some j),
...
C  ======= =======
C
C  A rough bound on x is computed; if that is less than overflow, DTPSV
...
C                                   1<=i< j
C
C  Since |x(j)| <= M(j), we use the Level 2 BLAS routine DTPSV if the
...
C                      1<=i<=j
C
C  and we can safely call DTPSV if 1/M(n) and 1/G(n) are both greater
...
C     .. External Subroutines ..
      EXTERNAL          DAXPY, DSCAL, DTPSV, F06AAZ
...
C
     ...
     *         .AND. .NOT. (NORMIN.EQ.'N' .OR. NORMIN.EQ.'n')) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07UGZ/DLATPS',-INFO)
         RETURN
      END IF
C
...
C     Level 2 BLAS routine DTPSV can be used.
C
      J = IDAMAX(N,X,1)
      XMAX = ABS(X(J))
      XBND = XMAX
