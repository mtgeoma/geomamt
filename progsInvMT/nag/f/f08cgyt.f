      SUBROUTINE F08CGY(SIDE,TRANS,M,N,K,A,LDA,TAU,C,LDC,WORK,LWORK,
     *                  INFO)
C     MARK 16 RELEASE. NAG COPYRIGHT 1992.
C     ENTRY             DORMQL(SIDE,TRANS,M,N,K,A,LDA,TAU,C,LDC,WORK,
C    *                  LWORK,INFO)
C
C  Purpose
C  =======
C
C  DORMQL overwrites the general real m by n matrix C with
C
...
C     .. External Subroutines ..
      EXTERNAL          F06AAZ, F07ZAZ, F08AEX, F08AEY, F08CGZ
...
C
     ...
     *         (TRANS.EQ.'T' .OR. TRANS.EQ.'t')) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08CGY/DORMQL',-INFO)
         RETURN
      END IF
C
...
C
         CALL F08CGZ(SIDE,TRANS,M,N,K,A,LDA,TAU,C,LDC,WORK,IINFO)
      ELSE
C
C        Use blocked code
C
...
C
            CALL F08AEX('Backward','Columnwise',NQ-K+I+IB-1,IB,A(1,I),
     *                  LDA,TAU(I),T,LDT)
            IF (LEFT) THEN
C
...
C           Apply H or H'
C
            CALL F08AEY(SIDE,TRANS,'Backward','Columnwise',MI,NI,IB,
