...
C     ..
C
C  F06TTF performs the transformation
C
C     R := P*U*conjg( Q' )  when  SIDE = 'L' or 'l' (  Left-hand side )
C
C     R := Q*U*conjg( P' )  when  SIDE = 'R' or 'r' ( Right-hand side ),
C
C  where  U and R  are n by n upper triangular matrices, P  is a unitary
C  matrix,  consisting  of a  given  sequence of  plane rotations  to be
C  applied in planes k1 to k2, and Q is a unitary matrix consisting of a
C  sequence of plane rotations,  applied in planes  k1 to k2,  chosen to
C  make  R  upper triangular.
C
...
C
C     T( k ) = (  c( k )  conjg( s( k ) ) ),   with c( k ) real.
C              ( -s( k )         c( k )   )
C
...
C     .. Local Scalars ..
      COMPLEX*16         AIJ, FILL, STEMP, STEMPC, TEMP
...
C
            ...
            DO 10 I = K1, MIN( J - 1, K2 - 1 )
               A( I, J ) = DCONJG( S( I ) )*A( I + 1, J ) + C( I )*AIJ
...
C           columns.
C
            CALL F06CAF( A( J + 1, J + 1 ), FILL, CTEMP, STEMP )
            C( J ) = CTEMP
            S( J ) = -STEMP
...
C              element.
C
               CALL F06CAF( A( J, J ), FILL, C( J ), S( J ) )
            END IF
   60    CONTINUE
C
