...
C     .. Entry Points ..
      ENTRY             ZPBTRF(UPLO,N,KD,AB,LDAB,INFO)
C
C  Purpose
C  =======
C
C  ZPBTRF computes the Cholesky factorization of a complex Hermitian
...
C     .. External Subroutines ..
      EXTERNAL          ZGEMM, ZHERK, ZTRSM, F06AAZ, F07FRZ, F07HRZ,
     *                  F07ZAZ
...
C
     ...
     *    .AND. ( .NOT. (UPLO.EQ.'L' .OR. UPLO.EQ.'l'))) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07HRF/ZPBTRF',-INFO)
         RETURN
      END IF
C
...
C
         CALL F07HRZ(UPLO,N,KD,AB,LDAB,INFO)
      ELSE
C
C        Use blocked code
C
...
C
C           Compute the Cholesky factorization of a Hermitian band
...
C
               CALL F07FRZ(UPLO,IB,AB(KD+1,I),LDAB-1,II)
               IF (II.NE.0) THEN
                  INFO = I + II - 1
                  GO TO 300
               END IF
...
C                    Update A23
C
                     IF (I2.GT.0) CALL ZGEMM('Conjugate transpose',
     *                                       'No transpose',I2,I3,IB,
     *                                       -CONE,AB(KD+1-IB,I+IB),
...
C
     ...
     *                          -ONE,WORK,LDWORK,ONE,AB(KD+1,I+KD),
     *                          LDAB-1)
C
...
C              Factorize the diagonal block
C
               CALL F07FRZ(UPLO,IB,AB(1,I),LDAB-1,II)
               IF (II.NE.0) THEN
                  INFO = I + II - 1
                  GO TO 300
               END IF
...
C
                     CALL ZTRSM('Right','Lower','Conjugate transpose',
     *                          'Non-unit',I3,IB,CONE,AB(1,I),LDAB-1,
     *                          WORK,LDWORK)
C
C                    Update A32
C
                     IF (I2.GT.0) CALL ZGEMM('No transpose',
     *                                       'Conjugate transpose',I3,
     *                                       I2,IB,-CONE,WORK,LDWORK,
     *                                       AB(1+IB,I),LDAB-1,CONE,
