...
C     .. Entry Points ..
      ENTRY             DORGLQ(M,N,K,A,LDA,TAU,WORK,LWORK,INFO)
C
C  Purpose
C  =======
C
...
C     .. External Subroutines ..
      EXTERNAL          F06AAZ, F07ZAZ, F08AEX, F08AEY, F08AJZ
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08AJF/DORGLQ',-INFO)
         RETURN
      END IF
C
...
C
      IF (KK.LT.M) CALL F08AJZ(M-KK,N-KK,K-KK,A(KK+1,KK+1),LDA,TAU(KK+1)
     *                         ,WORK,IINFO)
C
      IF (KK.GT.0) THEN
C
...
C
               CALL F08AEY('Right','Transpose','Forward','Rowwise',
...
C
            CALL F08AJZ(IB,N-I+1,IB,A(I,I),LDA,TAU(I),WORK,IINFO)
C
