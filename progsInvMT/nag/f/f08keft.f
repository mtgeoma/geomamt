...
C     .. Entry Points ..
      ENTRY             DGEBRD(M,N,A,LDA,D,E,TAUQ,TAUP,WORK,LWORK,INFO)
C
C  Purpose
C  =======
C
C  DGEBRD reduces a real general m by n matrix A to upper or lower
...
C     .. External Subroutines ..
      EXTERNAL          DGEMM, F06AAZ, F07ZAZ, F08KEY, F08KEZ
...
C
      ...
      END IF
      IF (INFO.LT.0) THEN
         CALL F06AAZ('F08KEF/DGEBRD',-INFO)
         RETURN
      END IF
C
...
C
         CALL F08KEY(M-I+1,N-I+1,NB,A(I,I),LDA,D(I),E(I),TAUQ(I),TAUP(I)
...
C
      CALL F08KEZ(M-I+1,N-I+1,A(I,I),LDA,D(I),E(I),TAUQ(I),TAUP(I),WORK,
     *            IINFO)
      WORK(1) = IWS
      RETURN
C
C     End of F08KEF (DGEBRD)
C
      END
