      SUBROUTINE F08PEW(SIDE,M,N,V,TAU,C,LDC,WORK)
C     MARK 16 RELEASE. NAG COPYRIGHT 1992.
C     ENTRY             DLARFX(SIDE,M,N,V,TAU,C,LDC,WORK)
C
C  Purpose
C  =======
C
...
C        w := C'*v
C
         CALL DGEMV('Transpose',M,N,ONE,C,LDC,V,1,ZERO,WORK,1)
C
C        C := C - tau * v * w'
C
...
C        w := C * v
C
         CALL DGEMV('No transpose',M,N,ONE,C,LDC,V,1,ZERO,WORK,1)
C
C        C := C - tau * w * v'
C
