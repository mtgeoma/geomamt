...
C     .. Entry Points ..
      ENTRY             ZHERFS(UPLO,N,NRHS,A,LDA,AF,LDAF,IPIV,B,LDB,X,
     *                  LDX,FERR,BERR,WORK,RWORK,INFO)
C
C  Purpose
C  =======
C
C  ZHERFS improves the computed solution to a system of linear
C  equations when the coefficient matrix is Hermitian indefinite and
...
C     .. External Subroutines ..
      EXTERNAL          ZAXPY, ZCOPY, ZHEMV, F04ZCF, F06AAZ, F07MSF
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07MVF/ZHERFS',-INFO)
         RETURN
      END IF
C
...
C
            CALL F07MSF(UPLO,N,1,AF,LDAF,IPIV,WORK,N,INFO)
