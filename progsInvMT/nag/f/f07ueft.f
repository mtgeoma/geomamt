...
C     .. Entry Points ..
      ENTRY             DTPTRS(UPLO,TRANS,DIAG,N,NRHS,AP,B,LDB,INFO)
C
C  Purpose
C  =======
C
C  DTPTRS solves a real triangular system of linear equations with
C  multiple right hand sides, of the form
C
C     A * X = B  or  A' * X = B,
C
C  where A is a triangular matrix of order N stored in packed format,
C  A' is the transpose of A, and B is an N by NRHS matrix.  A check is
C  made to verify that A is nonsingular.
C
C  Arguments
C  =========
C
...
C
     ...
     *         THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07UEF/DTPTRS',-INFO)
         RETURN
      END IF
C
