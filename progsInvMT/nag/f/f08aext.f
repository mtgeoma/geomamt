      SUBROUTINE F08AEX(DIRECT,STOREV,N,K,V,LDV,TAU,T,LDT)
C     MARK 16 RELEASE. NAG COPYRIGHT 1992.
C     ENTRY             DLARFT(DIRECT,STOREV,N,K,V,LDV,TAU,T,LDT)
C
C  Purpose
C  =======
C
...
C
                  CALL DGEMV('Transpose',N-I+1,I-1,-TAU(I),V(I,1),LDV,
     *                       V(I,I),1,ZERO,T(1,I),1)
               ELSE
C
C                 T(1:i-1,i) := - tau(i) * V(1:i-1,i:n) * V(i,i:n)'
C
                  CALL DGEMV('No transpose',I-1,N-I+1,-TAU(I),V(1,I),
...
C
                     CALL DGEMV('Transpose',N-K+I,K-I,-TAU(I),V(1,I+1),
     *                          LDV,V(1,I),1,ZERO,T(I+1,I),1)
                     V(N-K+I,I) = VII
                  ELSE
                     VII = V(I,N-K+I)
                     V(I,N-K+I) = ONE
