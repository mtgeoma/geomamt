...
C     .. Entry Points ..
      ENTRY             DPORFS(UPLO,N,NRHS,A,LDA,AF,LDAF,B,LDB,X,LDX,
     *                  FERR,BERR,WORK,IWORK,INFO)
C
C  Purpose
C  =======
C
C  DPORFS improves the computed solution to a system of linear
C  equations when the coefficient matrix is symmetric positive definite
C  and provides error bounds and backward error estimates for the
C  solutions.
C
C  Arguments
C  =========
...
C     .. External Subroutines ..
      EXTERNAL          F04YCF, F06AAZ, F07FEF, DAXPY, DCOPY, DSYMV
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07FHF/DPORFS',-INFO)
         RETURN
      END IF
C
...
C
            CALL F07FEF(UPLO,N,1,AF,LDAF,WORK(N+1),N,INFO)
