...
C          A = P*L*U as computed by F07ARF.
...
C     .. External Subroutines ..
      EXTERNAL          ZGEMM, ZGEMV, ZSWAP, ZTRSM, F06AAZ, F07TWF,
     *                  F07ZAZ
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07AWF/ZGETRI',-INFO)
         RETURN
      END IF
C
...
C
      ...
      END IF
C
C     Form inv(U).  If INFO > 0 from F07TWF, then U is singular,
C     and the inverse is not computed.
C
      CALL F07TWF('Upper','Non-unit',N,A,LDA,INFO)
      IF (INFO.GT.0) RETURN
C
C     Determine the block size for this environment.
C
      CALL F07ZAZ(1,'F07AWF',NB,0)
      IF (NB.LE.1) NB = N
C
      IF (NB.LT.N) THEN
         LDWORK = N
...
C
            IF (J.LT.N) CALL ZGEMV('No transpose',N,N-J,-ONE,A(1,J+1),
...
C
            IF (J+JB.LE.N) CALL ZGEMM('No transpose','No transpose',N,
