...
C     .. Entry Points ..
      ENTRY             ZSYTRI(UPLO,N,A,LDA,IPIV,WORK,INFO)
C
C  Purpose
C  =======
C
C  ZSYTRI computes the inverse of a complex symmetric indefinite matrix
...
C          used to obtain the factor U or L as computed by F07NRF.
C
...
C     .. External Subroutines ..
      EXTERNAL          ZCOPY, ZSWAP, F07NWZ, F06AAZ
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07NWF/ZSYTRI',-INFO)
         RETURN
      END IF
C
...
C
            IF (K.GT.1) THEN
               ...
               CALL F07NWZ(UPLO,K-1,-ONE,A,LDA,WORK,1,ZERO,A(1,K),1)
...
C
            IF (K.GT.1) THEN
               ...
               CALL F07NWZ(UPLO,K-1,-ONE,A,LDA,WORK,1,ZERO,A(1,K+1),1)
...
C
            IF (K.LT.N) THEN
               ...
               CALL F07NWZ(UPLO,N-K,-ONE,A(K+1,K+1),LDA,WORK,1,ZERO,
     *                    A(K+1,K),1)
...
C
     ...
     *                    A(K+1,K),1)
               A(K,K) = A(K,K) - ZDOTU(N-K,WORK,1,A(K+1,K),1)
               A(K,K-1) = A(K,K-1) - ZDOTU(N-K,A(K+1,K),1,A(K+1,K-1),1)
               CALL ZCOPY(N-K,A(K+1,K-1),1,WORK,1)
               CALL F07NWZ(UPLO,N-K,-ONE,A(K+1,K+1),LDA,WORK,1,ZERO,
     *                    A(K+1,K-1),1)
               A(K-1,K-1) = A(K-1,K-1) - ZDOTU(N-K,WORK,1,A(K+1,K-1),1)
            END IF
C
