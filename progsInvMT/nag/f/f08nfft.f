...
C     .. Entry Points ..
      ENTRY             DORGHR(N,ILO,IHI,A,LDA,TAU,WORK,LWORK,INFO)
C
C  Purpose
C  =======
C
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08NFF/DORGHR',-INFO)
         RETURN
      END IF
C
...
C
         CALL DORGQR(NH,NH,NH,A(ILO+1,ILO+1),LDA,TAU(ILO),WORK,LWORK,
     *               IINFO)
      ELSE
         WORK(1) = 1
      END IF
