...
C     .. Entry Points ..
      ENTRY             ZGEQRF(M,N,A,LDA,TAU,WORK,LWORK,INFO)
C
C  Purpose
C  =======
C
...
C     .. External Subroutines ..
      EXTERNAL          F06AAZ, F07ZAZ, F08ASX, F08ASY, F08ASZ
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F08ASF/ZGEQRF',-INFO)
         RETURN
      END IF
C
...
C           A(i:m,i:i+ib-1)
C
            CALL F08ASZ(M-I+1,IB,A(I,I),LDA,TAU(I),WORK,IINFO)
            IF (I+IB.LE.N) THEN
C
...
C
      IF (I.LE.K) CALL F08ASZ(M-I+1,N-I+1,A(I,I),LDA,TAU(I),WORK,IINFO)
C
      WORK(1) = IWS
      RETURN
C
C     End of F08ASF (ZGEQRF)
C
      END
