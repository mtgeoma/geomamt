...
C     .. Entry Points ..
      ENTRY             ZPOTRI(UPLO,N,A,LDA,INFO)
C
C  Purpose
C  =======
C
C  ZPOTRI computes the inverse of a complex Hermitian positive definite
C  matrix A using the Cholesky factorization A = U'*U or A = L*L'
C  computed by F07FRF.
C
C  Arguments
C  =========
C
...
C          factorization A = U'*U or A = L*L', as computed by F07FRF.
...
C
     ...
     *    .AND. .NOT. (UPLO.EQ.'L' .OR. UPLO.EQ.'l')) THEN
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL F06AAZ('F07FWF/ZPOTRI',-INFO)
         RETURN
      END IF
C
...
C
      CALL F07TWF(UPLO,'Non-unit',N,A,LDA,INFO)
      IF (INFO.GT.0) RETURN
C
C     Form inv(U)*inv(U)' or inv(L)'*inv(L).
C
      CALL F07FWZ(UPLO,N,A,LDA,INFO)
C
      RETURN
C
C     End of F07FWF (ZPOTRI)
C
      END
