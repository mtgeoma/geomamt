      SUBROUTINE D02MVZ(NEQMAX,NY2DIM,MAXORD,CONST,TCRIT,HMIN,HMAX,H0,
     *                  MAXSTP,MXHNIL,NORM,RWORK,IERR,REPORT)
C     MARK 14 RELEASE. NAG COPYRIGHT 1989.
C**********************************************************************
C**   D02MVZ - ROUTINE CALLED BY D02MVF (DASSL METHOD SETUP)         **
C**            SETS UP WORK SPACE ARRAY RWORK(50+4*NEQMAX)           **
C**********************************************************************
C
C     .. Scalar Arguments ..
      DOUBLE PRECISION  H0, HMAX, HMIN, TCRIT
      INTEGER           IERR, MAXORD, MAXSTP, MXHNIL, NEQMAX, NY2DIM
      LOGICAL           REPORT
      CHARACTER*(*)     NORM
C     .. Array Arguments ..
...
C     .. Executable Statements ..
     ...
     *         NORM1.EQ.'d') THEN
      ...
      DO 20 I = 1, 3
         IF (CONST(I).LT.0.0D0) THEN
            IERR = 1
            IF (REPORT) THEN
               WRITE (REC(1),FMT=99999) I, I, CONST(I)
               CALL X04BAF(IDEV,REC(1))
            END IF
   ...
   20 CONTINUE
      ...
      RETURN
C
99999 FORMAT (' ** On entry, CONST(',I1,').lt.0.0 : CONST(',I1,') =',
     *       D13.5)
99998 FORMAT (' ** On entry, CONST(',I1,').lt.1.0 : CONST(',I1,') =',
     *       D13.5)
99997 FORMAT (' ** On entry, CONST(1).ge.CONST(2) or CONST(2).ge.CONST',
     *       '(3)')
99996 FORMAT (' ** On entry,  NORM is not valid : NORM = ',A1)
      END
