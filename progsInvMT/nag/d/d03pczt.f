      SUBROUTINE D03PCZ(PDEFCN,BNDARY,DUMFCN,DUMBND,NEQ,NEQMAX,T,TOUT,Y,
     *                  RTOL,ATOL,ITOL,ITRACE,SNORM,MATZ,RESID,MONITR,
     *                  WKMON,NWKMON,WD,NWD,WKJAC,LENWJ,IW,NIW,OPT,
     *                  LDERIV,ITASK,WKRES,NWKRES,PDEFN,BNDY,ODEFN,
     *                  MONFFD,MONFKB,IND,IFAIL1,IA,NIA,JA,NJA,JCEVL,
     *                  JACFUL,JACBND,JACSPS,IRES,IXFIX,NIXFIX,NW)
C     MARK 15 RELEASE. NAG COPYRIGHT 1991.
C     MARK 16 REVISED. IER-980 (JUN 1993).
C     MARK 17 REVISED. IER-1548 (JUN 1995).
C----------------------------------------------------------------------
C     VP changed.
C++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C
C          PARAMETER LIST
C          --------------
C  The following variables are used to communicate with D03PCZ.
...
C      EXTERNAL RESID, MONITR, JAC, PDEFN, BNDY, ODEFN, MONFFD, MONFKB
C
...
C    stopped by the user in subroutine RESID or MYMNTR (see below),
...
C   possible values are  'A', 'M' and 'D' . where 'D'
...
C                                    NIW =  25*NEQMAX + 23
C
C  OPT - DOUBLE PRECISION array of size 22
...
C
C     where in the case of the D03PHF routine the discretisation
c     workspace size is given by:
C
C      NWKRES  = (16 + 2*NPDE + 6*NXI + MAXNPT) * NPDE +
C                 7 * MAXNPT + NXI + NV + 2*NXFIX
C
...
C
C  MONFFD/MONFKB
C   Names of subroutines used to define the monitor function used in
C   remeshing (for D03PPF and D03PRF respectively).
C   These names are only passed into the monitor routine and are not
C   used elsewhere. They must both be declared as EXTERNAL in the
C   calling program.
C
C  IND
C   Error/status indicator in the call to D03PCZ.
C
C  IFAIL1 - INTEGER
C   On entry
C           = 0
...
C     .. Subroutine Arguments ..
      EXTERNAL          BNDARY, BNDY, DUMBND, DUMFCN, JACBND, JACFUL,
     *                  JACSPS, MONFFD, MONFKB, MONITR, ODEFN, PDEFCN,
     *                  PDEFN, RESID
C     .. Scalars in Common ..
...
C     .. Local Scalars ..
     ...
     *                  ISPLIT, ITOTIW, ITOTW, J, JACPOS, JTRACE, LACOR,
     *                  LENYD, LENYS, LEWT, LIWREQ, LIWUSD, LRWREQ,
     *                  LRWUSD, LSAVR, MAXORD, MAXSTP, MXHNIL, NBLOCK,
     *                  NGP, NLU, NNZ
      LOGICAL           LBLOCK, PETZLD
      CHARACTER         JCEVAL, METOD1, METOD2
      CHARACTER*200     ERRMSG
C     .. Local Arrays ..
      DOUBLE PRECISION  CONST(6)
C     .. External Subroutines ..
      EXTERNAL          D02MWY, D02NNF, D02NNQ, D02NRF, D02NSF, D02NTF,
     *                  D02NUF, D02NVF, D02NXF, X04ABF
...
C     .. Save statement ..
      SAVE              /AD03PC/, /FD02NM/, /WD03PC/, /AD02NM/, /XD03PC/
...
C
         ...
         END IF
C
C ... Test integer workspace...
C
C         IF (ITOTIW.GT.NIW) THEN
C            GO TO 380
C         END IF
C
...
C
     ...
     *                  HMIN,HMAX,H0,MAXSTP,MXHNIL,SNORM,WD(IPOSRW),
     *                  IFAIL1)
C
         ELSE IF (OPT(1).EQ.2.D0) THEN
...
C
  160 CONTINUE
      CALL MONITR(PDEFCN,BNDARY,DUMFCN,DUMBND,NEQ,WD(IPOSRW+TN-1),
     *            WD(IPOSRW+HU-1),WD(IPOSRW+H-1),Y,WD(IPOSYD),WD(IPOSYS)
     *            ,NEQMAX,WD(IPOSRW+LSAVR-1),WD(IPOSRW+LEWT-1),WKRES,
     *            NWKRES,WKMON,NWKMON,IMON,INLN,WD(IPOSRW+HMIN1-1),
     *            WD(IPOSRW+HMAX1-1),MONFFD,MONFKB,PDEFN,BNDY,IRES,
     *            IXFIX,NIXFIX)
C VP NOV94
      IF (IMON.EQ.-2) THEN
...
C
      IF (IIFLAG.EQ.1) THEN
         ERRMSG =
     *' The flux function R was found to                              il
     *legally depend on time derivatives.'
         CALL D02NNQ(ERRMSG,1,0,0,0,0,0.0D0,0.0D0)
         IFAIL1 = 14
         GO TO 380
      END IF
