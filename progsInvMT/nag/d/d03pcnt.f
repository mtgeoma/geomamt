      SUBROUTINE D03PCN(PHFPDE,PHFBND,PDEPCF,BNDPCF,NEQN,T,HLAST,H,Y,
...
C
C ----------------------------------------------------------------------
C     MONITOR routine for remeshing (Finite Differences)
C ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C     .. Scalar Arguments ..
...
C     .. Subroutine Arguments ..
      EXTERNAL          BNDPCF, BNDR, MONFFD, MONFKB, PDEFN, PDEPCF,
     *                  PHFBND, PHFPDE
C     .. Scalars in Common ..
...
C     .. External Subroutines ..
      EXTERNAL          D02NNN, D03PCW, D03PRH, D03PRK, D03PRM, D03PRP,
     *                  D03PRQ, D03PRS, X04ABF, X04BAF
C     .. Common blocks ..
...
C     In which case CONST criterion may be over or under satisfied
      ...
      IFAIL = 0
C
C     Calculate flux values for use in MONFFD
      MIDNPT = NPTS + 1
      IF (PDCODE.EQ.'SPKEEL') THEN
         IF (NV.EQ.0) THEN
            DV(1) = 0.0D0
            CALL D03PRS(PHFPDE,PDEPCF,RESWK(I8),T,Y,NPDE,NPTS,RESWK(J3),
     *                  RESWK(J4),RESWK(J6),RESWK(J7),RESWK(I9),DV,NV,
     *                  RESWK(I7),PDEFN)
         ELSE
            CALL D03PRS(PHFPDE,PDEPCF,RESWK(I8),T,Y,NPDE,NPTS,RESWK(J3),
     *                  RESWK(J4),RESWK(J6),RESWK(J7),RESWK(I9),Y(NVST),
     *                  NV,RESWK(I7),PDEFN)
         END IF
         CALL MONFFD(T,NIP,NPDE,RESWK(I8),Y,RESWK(I9),RESWK(I12))
      ELSE
...
C
      IF (RMTYPE.EQ.'REMSET') THEN
         ...
         IF (I.EQ.0) THEN
C           New mesh not needed but redefine integration formula coeffs
C           as the workspace containing them was overwritten .
            IF (PDCODE.EQ.'SPKEEL') THEN
               CALL D03PCW(RESWK(I8),NPTS,M,RESWK(I9),RESWK(I10),
     *                     RESWK(I11),RESWK(I12),RESWK(I13),RESWK(K2),
     *                     RESWK(K3))
            END IF
C
            IMON = 1
