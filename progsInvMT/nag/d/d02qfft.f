...
C     .. Scalar Arguments ..
      ...
      INTEGER           IFAIL, LIWORK, LRWORK, NEQF, NEQG
      LOGICAL           ROOT
C     .. Array Arguments ..
...
C     .. Arrays in Common ..
     ...
     *                  SIG(13), V(12), W(12)
      INTEGER           IV(10)
C     .. Local Scalars ..
      DOUBLE PRECISION  GWANT, TSTOP, TWANT
      INTEGER           ERSTAT, I, IATOL, IDID, IER, IGNEW, IGOLD, IGP,
     *                  IGSC, IGV, IINDXG, IMMR, INEEDG, IP, IPHI,
     *                  IPHI3N, IPHI4N, IPHI5N, IPRO, IREVCM, IROD,
     *                  IRTOL, ISEE, ITGV, ITK, ITLB, ITRB, IWT, IYP,
     *                  IYPOUT, IYY, KWANT, NEQGCP, NREC, STATE
      LOGICAL           LOOP, VECTOL
C     .. Local Arrays ..
...
C     .. Common blocks ..
     ...
     *                  NEWGEQ, SEARCH, PSERCH
      COMMON            /LD02QF/TCHECK
C     .. Save statement ..
      SAVE              /AD02QF/, /BD02QF/, /LD02QF/, LOOP
C     .. Data statements ..
      DATA              LOOP/.FALSE./
...
C
      ...
      IDID = 0
   20 CONTINUE
      CALL D02QFX(IREVCM,TWANT,KWANT,GWANT,NEQF,T,Y,TOUT,RWORK(IRTOL),
...
C
   40 CONTINUE
      CALL F(NEQF,TWANT,Y,RWORK(IYPOUT))
      GO TO 20
   60 CONTINUE
      CALL F(NEQF,TWANT,RWORK(IYY),RWORK(IPHI5N))
      GO TO 20
   80 CONTINUE
      CALL F(NEQF,TWANT,RWORK(IPHI3N),RWORK(IPHI4N))
      GO TO 20
  100 CONTINUE
      CALL F(NEQF,TWANT,RWORK(IP),RWORK(IYP))
      GO TO 20
  120 CONTINUE
      CALL F(NEQF,TWANT,RWORK(IYY),RWORK(IYP))
      GO TO 20
  140 CONTINUE
      DO 160 I = 1, NEQGCP
         RWORK(IGOLD+I-1) = G(NEQF,TWANT,Y,RWORK(IYPOUT),I)
  160 CONTINUE
      GO TO 20
  180 CONTINUE
      GWANT = G(NEQF,TWANT,Y,RWORK(IYPOUT),KWANT)
      GO TO 20
C
  200 CONTINUE
      XSAVE = RWORK(TCURSV)
...
99996 FORMAT (' ** The dimension of RWORK supplied to D02QFF is not th',
     *       'at supplied to D02QWF:')
99995 FORMAT ('    LRWORK in D02QWF was ',I16,', LRWORK in D02QFF is ',
     *       I16,'.')
99994 FORMAT (' ** The dimension of IWORK supplied to D02QFF is not th',
     *       'at supplied to D02QWF:')
99993 FORMAT ('    LIWORK in D02QWF was ',I16,', LIWORK in D02QFF is ',
     *       I16,'.')
99992 FORMAT (' ** The value of NEQF supplied to D02QFF is not that su',
     *       'pplied to D02QWF:')
99991 FORMAT ('    NEQF in D02QWF was ',I16,', NEQF in D02QFF is ',I16,
     *       '.')
...
99986 FORMAT (' ** The positive value of NEQG,',I16,', indicating root',
     *       'finding required')
99985 FORMAT ('    conflicts with the positive value of NEQG,',I8,', s',
     *       'upplied to D02QWF.')
