...
C
  ...
  100 CALL RESID(NEQ,RWORK(TN),Y,RWORK(LSAVR),YDOTI,IRES)
      GO TO 20
C**********************************************************************C
C     SHELL USERS MUST REWRITE THE FOLLOWING REVERSE COMMUNICATION     C
C     CALL FOR JACOBIAN FORMING WITH OWN LINEAR ALGEBRA MODULE         C
C**********************************************************************C
  120 CALL D02NRF(J,IPLACE,INFORM)
      IF (IPLACE.EQ.1) THEN
         CALL JAC(NEQ,RWORK(TN),Y,YDOTI,RWORK(H),RWORK(EL0),J,
     *            RWORK(LSAVR))
      ELSE
         CALL JAC(NEQ,RWORK(TN),Y,YDOTI,RWORK(H),RWORK(EL0),J,
     *            RWORK(LACOR))
