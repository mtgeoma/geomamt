      SUBROUTINE D03PFN(NEQN,T,HLAST,H,Y,YDOT,YSAVE,NYH,R,ACOR,RESWK,
     *                  NRESWK,WKMON,NWKMON,IMON,INLN,HMIN,HMAX,MONITF,
     *                  IRES,IXFIX,NIXFIX)
C     MARK 17 RELEASE. NAG COPYRIGHT 1995.
C
C ----------------------------------------------------------------------
C     MONITOR routine for remeshing (upwind scheme routines)
C ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C     .. Scalar Arguments ..
...
C     .. Scalars in Common ..
     ...
     *                  IPMINF, ITRACE, J1, J10, J11, J12, J13, J14,
     *                  J15, J16, J17, J18, J2, J3, J4, J5, J6, J7, J8,
     *                  J9, K2, K3, KCUR, M, MAXNPT, NINTER, NOPUSR,
     *                  NPDE, NPTS, NRMESH, NV, NVST, NXFIX, NXI
      LOGICAL           REMESH
      CHARACTER*6       PDCODE, RMTYPE
C     .. Arrays in Common ..
      INTEGER           NDUM(6)
C     .. Local Scalars ..
      INTEGER           I, IFAIL, K, N, NF, NIP, NOLD, NOP
      CHARACTER*80      REC
C     .. External Subroutines ..
      EXTERNAL          D02NNN, D03PCW, D03PRH, D03PRK, D03PRM, D03PRQ,
     *                  D03PRR, X04ABF, X04BAF
C     .. Common blocks ..
...
C                   From RESWK(I11) onwards the workspace is also used i
...
C     In which case CONST criterion may be over or under satisfied
      ...
      IFAIL = 0
C
      CALL MONITF(T,NIP,NPDE,RESWK(I8),Y,RESWK(I12))
...
C
      IF (RMTYPE.EQ.'REMSET') THEN
         ...
         IF (I.EQ.0) THEN
C           New mesh not needed but redefine integration formula
C           coeffs as the workspace containing them was overwritten.
            CALL D03PCW(RESWK(I8),NPTS,M,RESWK(I9),RESWK(I10),RESWK(I11)
     *                  ,RESWK(I12),RESWK(I13),RESWK(K2),RESWK(K3))
            IMON = 1
            RETURN
         END IF
      END IF
...
C-----------------------------------------------------------------------
      CALL D03PRR(RESWK(I10),R,NOP,RESWK(I8),M,Y,NIP,NPDE,IFAIL,
     *            RESWK(I11),IXFIX,NXFIX)
      CALL D03PRQ(Y,R,N,NOLD,NV)
C-----------------------------------------------------------------------
C     Interpolate YDOT (YDOT is overwritten with new YDOT from R.
C-----------------------------------------------------------------------
      CALL D03PRR(RESWK(I10),R,NOP,RESWK(I8),M,YDOT,NIP,NPDE,IFAIL,
     *            RESWK(I11),IXFIX,NXFIX)
...
C        Interpolate YSAVE (and overwrite with new YSAVE)
         CALL D03PRR(RESWK(I10),R,NOP,RESWK(I8),M,YSAVE(1,K),NIP,NPDE,
...
C      UPDATE RESWK WITH XNP(NOP)
  ...
  100 CONTINUE
C
C     Call D03PCW to redefine space disc. formula coeffs as the
C     workspace containing them was overwritten in remeshing.
C
      CALL D03PCW(RESWK(I8),NPTS,M,RESWK(I9),RESWK(I10),RESWK(I11),
     *            RESWK(I12),RESWK(I13),RESWK(K2),RESWK(K3))
C
