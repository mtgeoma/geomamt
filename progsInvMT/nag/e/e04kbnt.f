      SUBROUTINE E04KBN(N,SFUN,MONIT,IPRINT,LOCSCH,INTYP,MINLIN,MAXFUN,
     *                  ETA,XTOL,STEPMX,FEST,IBOUND,BL,BU,X,HESL,LH,
     *                  HESD,ISTATE,F,G,IW,LIW,W,LW,IFAIL)
C
C     MARK 16 RELEASE. NAG COPYRIGHT 1993.
C
C     This routine originally called E04KBF.
C
C     **************************************************************
C
C     E04KBN IS A COMPREHENSIVE QUASI-NEWTON ALGORITHM FOR FINDING
...
C     SUBROUTINE SFUN TO COMPUTE THE FUNCTION VALUE AND/OR GRADIENT
C     VECTOR. (NOTE THAT, FOR CONSISTENCY WITH OTHER E04
C     DOCUMENTATION, THE NAME FUNCT IS USED INSTEAD OF SFUN IN THE
C     WRITE-UP.) THE ARRAYS W AND IW ARE USED AS WORK-SPACE BY
C     E04KBN AND MUST BE DIMENSIONED AT LEAST 9*N AND 2,
C     RESPECTIVELY. ON EXIT, E04KBN GIVES THE ESTIMATED POSITION OF
C     THE CONSTRAINED MINIMUM IN X AND THE LOWEST FUNCTION VALUE IN
C     F.
C
C     THE ROUTINE IS ESSENTIALLY IDENTICAL TO THE SUBROUTINE BCQNDR
C     IN THE NPL ALGORITHMS LIBRARY (REF. NO. E4/33/F). E04KBN WILL
C     NORMALLY BE CALLED WITH E04JBQ OR E04LBS (WHICH CALL E04ABZ OR
C     E04BBZ, RESPECTIVELY) AS THE PARAMETER MINLIN.
C
...
C     BY E04KBN (PARTICULARLY WHEN N IS LARGE) BY PUTTING THE BODIES
...
C     .. Local Scalars ..
      DOUBLE PRECISION  ALPHA, BNDMAX, BOUNDK, CHKEPS, COND, D, ENDEPS,
     *                  EPS, EPSA, EPSEPS, EPSMCH, EPSR, FM, FNEW,
     *                  GNEWTP, GPJNRM, GTG, GTP, OLDF, PCOND, PEPS,
     *                  PNORM, RMAX, RTEPS, RTOL, RTOLSQ, SAVKBD, SPE,
     *                  T, TEST, TNYTOL, TOLABS, TOLAX, TOLEPS, TOLREL,
     *                  TOLRTE, TOLRX, TSQEPS, TSQPEP, U, XNORM
      INTEGER           I, IFLAG, IGA, IGAOLD, INEGLM, INTYPE, IP, IPA,
     *                  J, JFIX, JGA, JGAOLD, LCOUNT, LHPROJ, LW1, LW2,
     *                  LW3, LW4, LW5, NEQUAL, NFREE, NFTOTL, NH, NITER,
     *                  NUMF, NUMNEG, NWHY
...
C     .. External Subroutines ..
      EXTERNAL          E04JBR, E04JBS, E04JBU, E04JBV, E04JBX, E04JBY,
     *                  E04JBZ, E04KBP, E04KBV, E04KBW, E04KBX, E04KBY,
     *                  E04KBZ, E04LBU, E04LBV, E04LBW, E04LBX, E04LBY,
     *                  E04LBZ, F04AQZ, X02ZAZ
...
C
     ...
     *    1 .OR. IBOUND.LT.0 .OR. IBOUND.GT.4) GO TO 640
      IF (IBOUND.NE.3) GO TO 20
      IF (BL(1).GT.BU(1)) GO TO 640
   20 IF (IBOUND.NE.0) GO TO 60
      DO 40 J = 1, N
         IF (BL(J).GT.BU(J)) GO TO 640
   40 CONTINUE
   60 CONTINUE
      NWHY = 0
C
...
C     ITS INDEX IN THE PERMUTATION OF FREE VARIABLES. NFREE RETURNS
C     THE TOTAL NUMBER OF FREE VARIABLES, THAT IS, THOSE WITH
C     POSITIVE ENTRIES IN ISTATE.
C
      IF (INTYPE.LE.1) CALL E04LBY(N,RTEPS,X,BL,BU,NFREE,ISTATE,NEQUAL)
      XNORM = SQRT(DDOT(N,X,1,X,1))
      IF (INTYPE.NE.1 .AND. INTYPE.NE.2) GO TO 80
C
C     PUT THE INITIAL FUNCTION VALUE, IF KNOWN, IN FNEW. IF THERE
C     ARE NO FREE VARIABLES, CHECK THE LAGRANGE MULTIPLIERS
C     FOR ANY FIXED VARIABLES THAT ARE NOT CONSTANTS.
...
C
      FNEW = F
      IF (NEQUAL.EQ.N) GO TO 620
      IF (NFREE.EQ.0) GO TO 480
      IF (INTYPE.EQ.2) GO TO 120
      CALL E04LBX(N,ISTATE,G,HESD,HESL,LH,W(IGA),HESD,HESL,LH,LHPROJ)
      GTG = DDOT(NFREE,W(IGA),1,W(IGA),1)
      GO TO 140
C
C     IF NECESSARY, CALL SFUN TO COMPUTE THE INITIAL FUNCTION VALUE
C     AND THE GRADIENT. THE SETTING OF IW(1), WHICH MUST NOT BE
C     CHANGED, ENABLES THE LIBRARY-USER TO DETERMINE FROM WHICH
C     SUBROUTINE SFUN IS CALLED. A NEGATIVE VALUE OF IFLAG INDICATES
C     FAILURE IN THE USER-SUPPLIED ROUTINE SFUN.
C
   80 IW(1) = 0
      IFLAG = 2
...
C
  120 CALL E04JBY(N,NFREE,ISTATE,G,W(IGA),GTG)
      LHPROJ = NFREE*(NFREE-1)/2
  140 IF (LHPROJ.EQ.0) LHPROJ = 1
C
...
C
      GPJNRM = SQRT(GTG)
      IF (IPRINT.LE.0) GO TO 220
      IF (MOD(NITER,IPRINT).NE.0) GO TO 220
...
C
  220 SPE = F06BLF(STEPMX,PNORM,FAIL)
C
...
C     VARIABLE FREE, E04JBY REDUCES THE PROJECTED GRADIENT AND
C     THE ROUTINE PROCEEDS IMMEDIATELY TO THE NEXT ITERATION.
C
      IFLAG = 0
      IW(1) = 0
...
C     ALPHA.
C
  260 ALPHA = E04JBT(FNEW,FM,GTP,SPE)
      IW(1) = -1
      U = 1.0D+0 + ABS(FNEW)
     ...
     *            NUMF,IFLAG,IW,LIW,W,LW)
      NFTOTL = NFTOTL + NUMF
      NITER = NITER + 1
      IF (IFLAG.EQ.0 .OR. IFLAG.EQ.2) GO TO 360
      IF (IFLAG.GT.0) GO TO 280
      NWHY = IFLAG
      GO TO 620
C
C     WHEN THE LINEAR SEARCH HAS FAILED TO FIND A LOWER POINT, THEN,
C     IF EITHER THE GRADIENT IS SMALL OR THE BOUND ON THE CONDITION
C     NUMBER OF THE PROJECTED HESSIAN HAS ALREADY BEEN REDUCED
C     UNSUCCESSFULLY, CHECK THE LAGRANGE MULTIPLIERS OF ANY FIXED
C     VARIABLES WHICH ARE NOT CONSTANTS.
C
  280 U = 1.0D+0 + ABS(FNEW)
      IF (BOUNDK.GE.1.0D+2 .AND. GTG.GE.PEPS*U*U) GO TO 320
      IF (BOUNDK.LE.1.0D+0) GO TO 300
      BOUNDK = 1.0D+0
      CALL E04JBX(NFREE,HESD,N,HESL,NH,COND)
      GO TO 160
  300 BOUNDK = SAVKBD
      NWHY = 3
      IF (N-NFREE.LE.NEQUAL) GO TO 580
      GO TO 480
C
C     OTHERWISE REDUCE BOUNDK, MODIFY THE CONDITION NUMBER OF THE
C     DIAGONAL FACTOR OF THE PROJECTED HESSIAN AND TRY AGAIN.
C
  320 BOUNDK = SQRT(BOUNDK)
      DO 340 I = 1, NFREE
         HESD(I) = SQRT(HESD(I))
  340 CONTINUE
      GO TO 160
C
C     THE LINEAR SEARCH HAS FOUND A SUFFICIENTLY LOWER POINT. IF
C     E04JBQ HAS BEEN USED, CALL SFUN TO COMPUTE THE NEW GRADIENT.
C
  360 IF (IFLAG.LE.1) GO TO 380
      IW(1) = 0
      IFLAG = 2
      CALL SFUN(IFLAG,N,X,FNEW,G,IW,LIW,W,LW)
      NFTOTL = NFTOTL + 1
...
C     FUNCTION IS RE-EVALUATED IF NECESSARY AND E04JBU REDUCES
C     THE CHOLESKY FACTORS OF THE PROJECTED HESSIAN. OTHERWISE
C     E04JBS RETURNS JFIX = 0.
C
  380 BOUNDK = SAVKBD
      XNORM = SQRT(DDOT(N,X,1,X,1))
      CALL E04JBS(N,RTEPS,NFREE,ISTATE,X,BL,BU,W(IP),JFIX,BOUNDK,RECALC)
      SAVKBD = BOUNDK
      IF (JFIX.EQ.0) GO TO 420
      IF ( .NOT. RECALC) GO TO 400
      IFLAG = 0
      IW(1) = 0
      CALL SFUN(IFLAG,N,X,FNEW,W(LW1),IW,LIW,W,LW)
      NFTOTL = NFTOTL + 1
      NWHY = IFLAG
      IF (IFLAG.LT.0) GO TO 620
  400 CALL E04JBU(N,NFREE,JFIX,HESL,NH,HESD,LHPROJ,W(LW1),IFLAG)
      IF (IFLAG.EQ.0) GO TO 420
      NWHY = 4
      GO TO 620
C
C     E04JBY PACKS THE NEW GRADIENT FOR THE SUBSPACE OF FREE
C     VARIABLES. IF THE SUBSPACE IS EMPTY, CHECK THE LAGRANGE
C     MULTIPLIERS OF ANY FIXED VARIABLES WHICH ARE NOT CONSTANTS.
C
  420 GTG = 0.0D+0
      IF (NFREE.EQ.0) GO TO 480
      CALL E04JBY(N,NFREE,ISTATE,G,W(IGA),GTG)
...
C
      GPJNRM = SQRT(GTG)
      IF (GPJNRM.GE.1.0D+1*XTOL) GO TO 440
      IFLAG = 0
      GO TO 460
  440 CALL E04KBY(NFREE,ALPHA,GTP,W(IGAOLD),GNEWTP,W(IGA),W(LW1),W(LW2),
     *            HESD,HESL,LH,IFLAG)
      IF (IFLAG.GE.0) GO TO 460
      NWHY = 4
      GO TO 620
  460 IF (IFLAG.GT.0) CALL E04KBZ(NFREE,BOUNDK,HESD,COND)
C
...
C
      ...
      IF (GTG.GT.CHKEPS*U*U .AND. D.GT.1.0D-1*TEST) GO TO 160
C
...
C
      IF (INEGLM.GT.0) GO TO 560
      IF ( .NOT. LM1POS .AND. (LSFAIL .OR. NFREE.EQ.0)) GO TO 540
      IF (NFREE.EQ.0) GO TO 620
      IF (LSFAIL) GO TO 580
      IF (GTG.LT.ENDEPS*U*U) GO TO 520
      IF (NITER.EQ.0) GO TO 500
      D = OLDF - FNEW
      U = 1.0D+0 + ABS(FNEW)
      IF (ALPHA*PNORM.LT.TOLRTE*(1.0D+0+XNORM) .AND. ABS(D)
     *    .LT.TSQEPS*U .AND. GTG.LT.TSQPEP*U*U) GO TO 520
  500 IF (NUMNEG.GT.0) CALL E04LBU(N,NUMNEG,ISTATE)
      GO TO 160
  520 IF ( .NOT. LM1POS) GO TO 540
      IF (LOCSCH) GO TO 580
      GO TO 620
C
...
C
  540 IW(1) = -5
     ...
     *            INEGLM,IW,LIW,W,LW)
      ...
      IF (INEGLM.EQ.0) GO TO 620
C
C     TAKE THE PERTURBED X AS THE STARTING POINT FOR THE NEXT
C     ITERATION. THE RE-EVALUATION OF THE FUNCTION AND THE GRADIENT
C     ENABLES THE LIBRARY-USER TO OBTAIN ADDITIONAL INFORMATION AT
C     THE NEW POINT.
C
      XNORM = SQRT(DDOT(N,X,1,X,1))
      IW(1) = 0
...
C     HESD.
C
  560 CALL E04KBV(N,INEGLM,G,NFREE,ISTATE,W(IGA),GTG,HESD,LHPROJ,BOUNDK,
     *            COND)
      SAVKBD = BOUNDK
      GO TO 160
C
C     THE LOCAL SEARCH ROUTINE E04KBP IS USED AS A FINAL CHECK.
C
  580 IF (IPRINT.LE.0) GO TO 600
      GPJNRM = SQRT(GTG)
      CALL E04LBW(HESD,NFREE,RMAX,IFLAG,PCOND)
      IF (IFLAG.EQ.1) PCOND = RMAX
      CALL MONIT(N,X,FNEW,G,ISTATE,GPJNRM,PCOND,.TRUE.,NITER,NFTOTL,IW,
     *           LIW,W,LW)
  600 CALL E04KBP(N,NFREE,MAXFUN,FM,RTOLSQ,TOLEPS,ENDEPS,STEPMX,RTEPS,
     *            SFUN,MINLIN,X,BL,BU,FNEW,ISTATE,G,GTG,W(IGA),W(IP),
     *            HESL,LH,LHPROJ,HESD,BOUNDK,NFTOTL,LCOUNT,NWHY,FINISH,
     *            IW,LIW,W,LW)
      XNORM = SQRT(DDOT(N,X,1,X,1))
      IF ( .NOT. FINISH) GO TO 160
C
C     TERMINATION OF ALGORITHM
C
  620 F = FNEW
      IF (IPRINT.LT.0) GO TO 640
      GPJNRM = SQRT(GTG)
      COND = 0.0D+0
      IFLAG = 0
     ...
     *           LIW,W,LW)
  640 IF (NWHY.EQ.0) GO TO 660
      IFAIL = P01ABF(IFAIL,NWHY,SRNAME,0,P01REC)
      RETURN
  660 IFAIL = 0
      RETURN
C
C     END OF E04KBN (BCQNDR)
C
      END
