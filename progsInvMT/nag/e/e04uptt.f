      SUBROUTINE E04UPT(NEEDFD,INFORM,M,N,NCNLN,LDCJ,LDCJU,LDFJ,LDFJU,
     *                  NFUN,NGRAD,NEEDC,CONFUN,OBJFUN,ALFA,ALFBND,
     *                  ALFMAX,ALFSML,DXNORM,EPSRF,ETA,GDX,GRDALF,GLF1,
     *                  GLF,OBJF,OBJALF,QPCURV,XNORM,C,CJAC,CJACU,CJDX,
     *                  CMUL1,CMUL,CS1,CS,DX,DLAM,DSLK,F,FJAC,FJACU,
     *                  QPMUL,RHO,SLK1,SLK,X1,X,W,LENW,IUSER,USER)
C     MARK 14 RELEASE.  NAG COPYRIGHT 1989.
C     MARK 16 REVISED. IER-1100 (JUL 1993).
C     MARK 17 REVISED. IER-1620 (JUN 1995).
C
C     ==================================================================
C     E04UPT finds the steplength ALFA that gives sufficient decrease in
...
C     This version of E04UPT dated  4-Nov-91.
C     ==================================================================
C
C     .. Parameters ..
      INTEGER           LENLS
...
C     .. Array Arguments ..
     ...
     *                  DSLK(*), DX(N), F(*), FJAC(LDFJ,*),
     *                  FJACU(LDFJU,*), QPMUL(*), RHO(*), SLK(*),
     *                  SLK1(*), USER(*), W(LENW), X(N), X1(N)
      INTEGER           IUSER(*), NEEDC(*)
...
C     .. Local Scalars ..
     ...
     *                  TOLTNY
      INTEGER           IPRINT, J, LC, LCJDX, LF, LFJDX, LWORK, MAXF,
     *                  MODE, NSTATE, NUMF
...
C
         IF (M.GT.0) CALL DCOPY(M,W(LF),1,F,1)
C
         IF ( .NOT. NEEDFD) THEN
            GDX = TGDX
            GLF = TGLF
...
C        ------------------------------------------------------------
         IF (M.GT.0) THEN
            ...
            IF (MODE.LT.0) GO TO 60
            TOBJ = HALF*DDOT(M,W(LF),1,W(LF),1)
         END IF
C
         IF (NCNLN.GT.0) THEN
            TOBJM = TOBJ - FTERM
...
C        ------------------------------------------------------------
         IF ( .NOT. NEEDFD) THEN
            IF (M.GT.0) THEN
               CALL DGEMV('Normal',M,N,ONE,FJACU,LDFJU,DX,1,ZERO,
     *                    W(LFJDX),1)
               GTRY = DDOT(M,W(LFJDX),1,W(LF),1)
            END IF
            TGDX = GTRY
            TGLF = GTRY
C
...
C
               CALL F06FCF(NCNLN,RHO,1,W(LWORK),1)
               GTRY = GTRY + SCALE*DDOT(NCNLN,W(LWORK),1,CS,1)
C
