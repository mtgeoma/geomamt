      SUBROUTINE E01BFZ(N,X,F,D,INCFD,M,PX,PF,IERR)
...
C
C     PX -- (input) real array of points at which the function is to be
C           evaluated.
C           Notes:
...
C     .. Array Arguments ..
      DOUBLE PRECISION  D(INCFD,N), F(INCFD,N), PF(M), PX(M), X(N)
C     .. Local Scalars ..
...
C     Assume that arguments have been checked by E01BFF.
C
...
C           Evaluate cubic at PX(I),  I = JFIRST (1) J-1 .
            CALL E01BFY(X(IR-1),X(IR),F(1,IR-1),F(1,IR),D(1,IR-1),D(1,
     *                  IR),NJ,PX(JFIRST),PF(JFIRST),NEXT,IERC)
            IF (IERC.GT.0) THEN
               GO TO 220
            ELSE
C
