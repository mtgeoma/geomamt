      SUBROUTINE E04UPZ(NAMED,NAMES,UNITQ,INFORM,MAJITS,M,N,NCLIN,NCNLN,
     ...
     *                  F,FJAC,FJACU,FEATOL,R,X,IW,W,LENW,IUSER,USER)
C     MARK 14 RELEASE.  NAG COPYRIGHT 1995.
C     MARK 16 REVISED. IER-1105 (JUL 1993).
C     MARK 17 REVISED. IER-1625 (JUN 1995).
C
C     ******************************************************************
C     E04UPZ  is the core routine for  E04UPF,  a sequential quadratic
...
C     This version of E04UPZ dated  23-Dec-92.
C     ******************************************************************
C
C     .. Parameters ..
      INTEGER           LENLS
...
C     .. Array Arguments ..
     ...
     *                  FJACU(LDFJU,*), R(LDR,*), USER(*), W(LENW), X(N)
...
C     .. Local Scalars ..
     ...
     *                  GRDALF, GTEST, GZNORM, OBJALF, OBJSIZ, QPCURV,
     *                  ROOTN, RTFTOL, RTMAX, XSIZE
     ...
     *                  LNEEDC, LQPTOL, LQRWRK, LRHO, LRLAM, LRPQ, LSLK,
     *                  LSLK1, LT, LVIOLN, LWRK1, LWRK2, LWRK3, LWTINF,
     *                  LX1, LZY, MAJIT0, MINITS, MNR, MNRSUM, MODE,
     *                  MSGQP, NCQP, NCTOTL, NL, NLNACT, NLSERR, NMAJOR,
     *                  NMINOR, NPLIN, NQPERR, NQPINF, NSTATE, NUMINF,
     *                  NVIOL
...
C     .. External Subroutines ..
     ...
     *                  E04UPP, E04UPS, E04UPT, E04UPW, F06DBF, F06FBF,
     *                  F06FLF, F06QFF, X04BAF, X04BAY
...
C
         END IF
C
         CALL DGEMV('Transpose',M,N,ONE,FJAC,LDFJ,F,1,ZERO,W(LGQ),1)
C
...
C        ============================================================
         ...
         ALFA = MIN(ALFLIM,ONE)
C
         CALL E04UPT(NEEDFD,NLSERR,M,N,NCNLN,LDCJ,LDCJU,LDFJ,LDFJU,NFUN,
     ...
     *               F,FJAC,FJACU,CLAMDA(NL),W(LRHO),W(LSLK1),W(LSLK),
     *               W(LX1),X,W,LENW,IUSER,USER)
C
C        ------------------------------------------------------------
C        E04UPT  sets NLSERR to the following values...
C
...
C
               ...
               IF (MODE.LT.0) GO TO 60
C
               CALL DGEMV('No Transpose',M,N,ONE,FJAC,LDFJ,W(LDX),1,
     *                    ZERO,W(LFJDX),1)
               GDX = DDOT(M,W(LFJDX),1,F,1)
               GLF2 = GDX
               IF (NCNLN.GT.0) THEN
...
C              ======================================================
               CALL E04UCL(MJRMSG,UNITQ,N,NCNLN,NFREE,NZ,LDCJ1,LDCJ,
     *                     LDZY,LDR,KX,ALFA,GLF1,GLF2,QPCURV,W(LCJAC1),
     *                     CJAC,W(LCJDX1),W(LCJDX),W(LCS1),W(LCS2),
     *                     W(LGQ1),W(LGQ),W(LHPQ),W(LRPQ),CLAMDA(NL),R,
...
C              Reset R'R  as J'J.
C              ------------------------------------------------------
               CALL E04UPS(N,LDZY,NFREE,UNITQ,KX,M,FJAC,LDFJ,R,LDR,
     *                     W(LZY),W(LWRK1))
