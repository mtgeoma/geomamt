...
C     .. Array Arguments ..
      ...
      INTEGER           IW(LIW)
C     .. Local Scalars ..
      DOUBLE PRECISION  BNDMAX, DELTA, EPSMCH, ETA, GTG, PEPS, RTEPS,
     *                  STEPMX, U, XI, XTOL
...
C     .. External Subroutines ..
      EXTERNAL          E04CGZ, E04DEZ, E04HCF, E04KDZ, E04LBR, E04LBS
...
C
      ETA = 1.0D-2
      IF (N.LE.20) ETA = 1.0D-1
      IF (N.LE.10) ETA = 5.0D-1
      IF (N.EQ.1) ETA = 0.0D+0
C
C     SET THE FINITE-DIFFERENCE INTERVAL FOR APPROXIMATING THE
C     HESSIAN MATRIX. DELTA WILL BE RESET TO SQRT(EPSMCH) WITHIN
C     E04LBR.
C
      DELTA = 0.0D+0
C
...
C
   20 CALL E04LBR(N,E04DEZ,E04KDZ,.FALSE.,E04CGZ,IPRINT,E04LBS,MAXFUN,
     *            ETA,XTOL,DELTA,STEPMX,IBOUND,BL,BU,X,W(KW1),LH,W(KW2),
     *            IW(KIW),F,G,IW,LIW,W,LW,NWHY)
      IF (NWHY.LE.2) GO TO 60
C
