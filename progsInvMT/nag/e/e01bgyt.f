      SUBROUTINE E01BGY(X1,X2,F1,F2,D1,D2,NE,XE,FE,DE,NEXT,IERR)
...
C
C     If only function values are required, use E01BFY instead.
C
C     ------------------------------------------------------------------
C
C     Parameters:
...
C                (Output arrays have not been changed if IERR .ne. 0)
C
C     ------------------------------------------------------------------
C
...
C     .. Scalar Arguments ..
      ...
      INTEGER           IERR, NE
C     .. Array Arguments ..
      DOUBLE PRECISION  DE(NE), FE(NE), XE(NE)
      INTEGER           NEXT(2)
C     .. Local Scalars ..
