...
C        ignoring zeros.
         ...
         DO 40 J = 1, N
            DO 20 I = 1, J - 1
               K = K + 1
               AA = MAX(ABS(DBLE(A(K))),ABS(DIMAG(A(K))))
...
C              Lower triangular matrix.
               OFFI = INDNT + RLBWID + 2
               DO 220 CL = CLEFT, CRIGHT
                  K = ((2*N-CL)*(CL-1))/2 + J
                  IF (ABOVE) THEN
                     WRITE (INFILE(OFFI:OFFI+NUMWID-1),FMT=FORM,ERR=160)
     *                 DBLE(A(K))
  160                CONTINUE
                     WRITE (INFBOT(OFFI:OFFI+NUMWID-1),FMT=FORM,ERR=180)
     *                 DIMAG(A(K))
  180                CONTINUE
                  ELSE
  ...
  220          CONTINUE
     ...
     *                 DBLE(DIAGEL)
  240                CONTINUE
                     WRITE (INFBOT(OFFI:OFFI+NUMWID-1),FMT=FORM,ERR=260)
     *                 DIMAG(DIAGEL)
  260                CONTINUE
                  ELSE
                     WRITE (INFILE(OFFI:OFFI+NUMWID-1),FMT=FORM,ERR=280)
     *                 DIAGEL
  280                CONTINUE
                  END IF
               END IF
...
C              Upper triangular matrix.
               ...
               IF (ABOVE) THEN
                  IF (PRDIAG) THEN
                     WRITE (INFILE(OFFI:OFFI+NUMWID-1),FMT=FORM,ERR=300)
     *                 DBLE(DIAGEL)
  300                CONTINUE
                     WRITE (INFBOT(OFFI:OFFI+NUMWID-1),FMT=FORM,ERR=320)
     *                 DIMAG(DIAGEL)
  320                CONTINUE
                  END IF
                  DO 380 CL = CLEFT, CRIGHT
                     OFFI = OFFI + NUMWID
                     WRITE (INFILE(OFFI:OFFI+NUMWID-1),FMT=FORM,ERR=340)
     *                 DBLE(A(((CL-1)*CL)/2+J))
  340                CONTINUE
                     WRITE (INFBOT(OFFI:OFFI+NUMWID-1),FMT=FORM,ERR=360)
     *                 DIMAG(A(((CL-1)*CL)/2+J))
  360                CONTINUE
  380             CONTINUE
               ELSE
                  IF (PRDIAG) WRITE (INFILE(OFFI:OFFI+NUMWID-1),
     *                FMT=FORM,ERR=400) DIAGEL
  400             CONTINUE
                  DO 440 CL = CLEFT, CRIGHT
                     OFFI = OFFI + NUMWID
                     WRITE (INFILE(OFFI:OFFI+NUMWID-1),FMT=FORM,ERR=420)
     *                 A(((CL-1)*CL)/2+J)
  420                CONTINUE
  440             CONTINUE
               END IF
...
C
  ...
  480 CONTINUE
C
  500 IFAIL = P01ABF(IFAIL,IERR,SRNAME,NREC,REC)
C
      RETURN
C
...
99996 FORMAT (1X,'** On entry, USEFRM is not valid : USEFRM = ''',A,
     *       '''.')
