...
C     .. Parameters ..
      CHARACTER*6       SRNAME
      PARAMETER         (SRNAME='C06HDF')
C     .. Scalar Arguments ..
      INTEGER           IFAIL, M, N
      CHARACTER*1       DIRECT, INIT
C     .. Array Arguments ..
      DOUBLE PRECISION  TRIG(2*N), WORK(1:M,0:N-1), X(1:M,0:N-1)
C     .. Local Scalars ..
...
C
   ...
   20       CONTINUE
            DO 60 J = 1, N - 1
               DO 40 I = 1, M
                  WORK(I,J) = SIN(DBLE(J)*PIBY2N)*(X(I,J)+X(I,N-J)) +
     *                        COS(DBLE(J)*PIBY2N)*(X(I,J)-X(I,N-J))
   40          CONTINUE
   60       CONTINUE
C
C           Transform the D array, stored in WORK, using X as
C           the workspace array
C
            CALL C06FPX(WORK,X,M,N,Q,NQ,TRIG)
C
C           Now extract the quarter-wave cosine transform from the
C           Fourier transform in WORK, putting the required transform
C           in X
C
            DO 80 I = 1, M
               X(I,0) = 0.5D0*WORK(I,0)
   80       CONTINUE
            DO 120 J = 1, (N-1)/2
               DO 100 I = 1, M
...
C
            DO 200 I = 1, M
               WORK(I,0) = 2.0D0*X(I,0)
  200       CONTINUE
            DO 240 K = 1, (N-1)/2
               DO 220 I = 1, M
                  WORK(I,K) = X(I,2*K) + X(I,2*K-1)
                  WORK(I,N-K) = X(I,2*K) - X(I,2*K-1)
  220          CONTINUE
  240       CONTINUE
            IF (MOD(N,2).EQ.0) THEN
               N2 = N/2
               DO 300 I = 1, M
                  WORK(I,N2) = 2.0D0*X(I,N-1)
  300          CONTINUE
            END IF
C
C           Calculate the D array by taking the inverse transform
C           of the Re + i*Im arrays
C
            JFAIL = 0
...
C
  ...
  320       CONTINUE
            DO 360 J = 1, N - 1
               DO 340 I = 1, M
                  X(I,J) = 0.5D0*(COS(DBLE(J)*PIBY2N)*(WORK(I,J)+WORK(I,
     *                     N-J))+SIN(DBLE(J)*PIBY2N)*(WORK(I,J)-WORK(I,
     *                     N-J)))
  340          CONTINUE
  360       CONTINUE
         END IF
