      SUBROUTINE Y07BEF(JOB,M,N,A,LDA,TITLE,NFIGB,NFIGA,NCOLS,NOUT)
C     MARK 13 RELEASE. NAG COPYRIGHT 1988.
C
C  Purpose
C  =======
C
C  Y07BEF prints out a matrix,  or its transpose,  using an  F format on
C  device  NOUT.  Each element is printed with  NFIGB figures before the
C  decimal point and  NFIGA  figures after the decimal point.  A heading
C  may be printed before the matrix.
C
...
C  then Y07BEF may be called as
C
C     CALL Y07BEF( 'No transpose', 6, 5, A, 10, 'Matrix A',
C    $             2, 5, 60, 6 )
C
C  To get  the default values for  NFIGB, NFIGA, NCOLS and NOUT,  Y07BEF
C  may be called as
C
C     CALL Y07BEF( 'Row by row', 6, 5, A, 10, 'Matrix A',
C    $             -1, -1, -1, -1, )
C
