      SUBROUTINE Y07BBF(N,X,INCX,TITLE,NFIGB,NFIGA,NCOLS,NOUT)
C     MARK 13 RELEASE. NAG COPYRIGHT 1988.
C
C  Purpose
C  =======
C
C  Y07BBF prints out the n element vector x, using an F format on device
...
C          width for each element of  x  will be  ( NFIGB + NFIGA + 3 ).
C          If  NFIGA  is not in the range  ( 0, 30 ) then the value 5 is
...
C  and the heading  'Vector x',  Y07BBF may be called as
C
C     CALL Y07BBF( 20, X, 1, 'Vector x', 2, 5, 60, 6 )
C
C  To get  the default values for  NFIGB, NFIGA, NCOLS and NOUT,  Y07BBF
C  may be called as
C
C     CALL Y07BBF( 20, X, 1, 'Vector x', -1, -1, -1, -1 )
C
