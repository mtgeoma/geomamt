      SUBROUTINE Y07BFF(JOB,N,A,TITLE,NSIG,NCOLS,NOUT)
C     MARK 13 RELEASE. NAG COPYRIGHT 1988.
C
C  Purpose
C  =======
C
C  Y07BFF  prints out the  triangular part  of  symmetric or  triangular
...
C  per line and the heading  'Lower triangle of A',  then  Y07BFF may be
C  called as
C
C     CALL Y07BFF( 'Row by row', 6, A, 'Lower triangle of A',
C    $             4, 60, 6 )
C
C  To get  the default values  for  NSIG, NCOLS and NOUT,  Y07BFF may be
C  called as
C
C     CALL Y07BFF( 'Row by row', 6, A, 'Lower triangle of A',
C    $             -1, -1, -1 )
