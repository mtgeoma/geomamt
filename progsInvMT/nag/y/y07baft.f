      SUBROUTINE Y07BAF(N,X,INCX,TITLE,NSIG,NCOLS,NOUT)
C     MARK 13 RELEASE. NAG COPYRIGHT 1988.
C
C  Purpose
C  =======
C
C  Y07BAF  prints out  the  n element vector x,  using  an  E format  on
C  device  NOUT. Each element is printed to  NSIG significant figures. A
C  heading may be printed before the vector.
C
C  Parameters
C  ==========
C
...
C          Unchanged on exit.
C
C  X     - REAL             array of DIMENSION at least ( max( 1, lx ) )
...
C          element of  x  will be  ( NSIG + 7 ).  If NSIG is outside the
C          range  ( 1, 92 )  then the value 7 is used in place of  NSIG.
C
C          Unchanged on exit.
C
C  NCOLS - INTEGER.
...
C  the heading  'Vector x',  Y07BAF may be called as
C
C     CALL Y07BAF( 20, X, 1, 'Vector x', 4, 60, 6 )
C
C  To get the  default values  for  NSIG, NCOLS and NOUT,  Y07BAF may be
C  called as
C
C     CALL Y07BAF( 20, X, 1, 'Vector x', -1, -1, -1 )
C
C  Nag Fortran 77 basic linear algebra routine.
C
C  -- Written on 6-December-1982.
