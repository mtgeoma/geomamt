      SUBROUTINE Y07CDF(JOB,M,N,A,LDA,TITLE,NSIG,NCOLS,NOUT)
C     MARK 13 RELEASE. NAG COPYRIGHT 1988.
C
C  Purpose
C  =======
C
C  Y07CDF prints out a matrix,  or its  conjugate transpose,  using an E
C  format on device  NOUT.  Each element is printed to  NSIG significant
C  figures. A heading may be printed before the matrix.
C
...
C           each element of A will be  2*( NSIG + 7 ). A comma separates
C           the real and imaginary parts. If  NSIG  is outside the range
...
C  Y07CDF may be called as
C
C     CALL Y07CDF( 'No transpose', 6, 5, A, 10, 'Matrix A', 4, 60, 6 )
C
C  To get  the default values  for  NSIG, NCOLS and NOUT,  Y07CDF may be
C  called as
C
C     CALL Y07CDF( 'No transpose', 6, 5, A, 10, 'Matrix A', -1, -1, -1 )
C
