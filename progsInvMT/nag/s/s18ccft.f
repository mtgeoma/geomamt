...
C     .. Parameters ..
      CHARACTER*6                      SRNAME
      PARAMETER                        (SRNAME='S18CCF')
C     .. Scalar Arguments ..
      DOUBLE PRECISION                 X
      INTEGER                          IFAIL
C     .. Local Scalars ..
      DOUBLE PRECISION                 EGAM, G, RYBIG, T, XBIG, XVSMAL,
     *                                 Y
C     .. Local Arrays ..
...
C     .. Data statements ..
C08   DATA XBIG, XVSMAL, EGAM /1.0D+10,1.0D-9,5.77215665D-1/
C08   DATA RYBIG /7.97884561D-1/
C09   DATA XBIG, XVSMAL, EGAM /1.0D+11,1.0D-10,5.772156649D-1/
C09   DATA RYBIG /7.978845608D-1/
C12   DATA XBIG, XVSMAL, EGAM /1.0D+14,1.0D-13,5.772156649015D-1/
C12   DATA RYBIG /7.978845608029D-1/
C15   DATA XBIG, XVSMAL, EGAM /1.0D+17,1.0D-16,5.772156649015329D-1/
C15   DATA RYBIG /7.978845608028654D-1/
      DATA XBIG, XVSMAL, EGAM /1.0D+19,1.0D-18,5.77215664901532861D-
     * 1/
      DATA RYBIG /7.97884560802865356D-1/
C19   DATA XBIG, XVSMAL, EGAM /1.0D+21,1.0D-20,5.7721566490153286061
C19  * D-1/
C19   DATA RYBIG /7.978845608028653559D-1/
C
C     XBIG IS CHOSEN SUCH THAT
C     IF(X.GE.XBIG) S18CCF = SQRT(PI/2X)
C     TO WITHIN MACHINE ACCURACY
C
C     XVSMAL IS CHOSEN SUCH THAT
C     IF(X.LE.XVSMAL) S18CCF = -(LOG(X/2) + EGAM)
C     TO WITHIN MACHINE ACCURACY
