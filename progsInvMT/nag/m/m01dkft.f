...
C
            ...
            IF (A.GE.C) THEN
               IF (A.EQ.C) THEN
                  DO 20 IH = M1 + 1, M2
                     IF (IM(IH,K).GT.IM(IH,J)) GO TO 40
                     IF (IM(IH,K).LT.IM(IH,J)) GO TO 60
   20             CONTINUE
               END IF
   40          IRANK(J) = K
               J = K
   ...
   60       IF (A.LE.B) THEN
               IF (A.EQ.B) THEN
                  DO 80 IH = M1 + 1, M2
                     IF (IM(IH,K).LT.IM(IH,I)) GO TO 100
                     IF (IM(IH,K).GT.IM(IH,I)) GO TO 120
   80             CONTINUE
                  GO TO 120
               END IF
  100          IRANK(K) = I
...
C
  ...
  140          I1 = I2
               I2 = IRANK(I1)
               IF (A.GT.IM(M1,I2)) GO TO 140
               IF (A.EQ.IM(M1,I2)) THEN
                  DO 160 IH = M1 + 1, M2
                     IF (IM(IH,K).LT.IM(IH,I2)) GO TO 180
                     IF (IM(IH,K).GT.IM(IH,I2)) GO TO 140
  160             CONTINUE
                  GO TO 140
               END IF
  180          CONTINUE
...
C
  220    ILIST = -1
         ...
         J = -LIST2
  240    K = I
         IF (IM(M1,I).GT.IM(M1,J)) K = J
         IF (IM(M1,I).EQ.IM(M1,J)) THEN
            DO 260 IH = M1 + 1, M2
               IF (IM(IH,I).GT.IM(IH,J)) GO TO 280
               IF (IM(IH,I).LT.IM(IH,J)) GO TO 300
  260       CONTINUE
            GO TO 300
  280       K = J
         END IF
...
C
  ...
  340    I = K
         K = IRANK(I)
         IF (K.LE.0) GO TO 480
         IF (A.GT.IM(M1,K)) GO TO 340
         IF (A.EQ.IM(M1,K)) THEN
            DO 360 IH = M1 + 1, M2
               IF (IM(IH,J).LT.IM(IH,K)) GO TO 380
               IF (IM(IH,J).GT.IM(IH,K)) GO TO 340
  360       CONTINUE
            GO TO 340
         END IF
  380    IRANK(I) = J
  ...
  420    J = K
         K = IRANK(J)
         IF (K.LE.0) GO TO 500
         IF (A.GT.IM(M1,K)) GO TO 420
         IF (A.EQ.IM(M1,K)) THEN
            DO 440 IH = M1 + 1, M2
               IF (IM(IH,I).LT.IM(IH,K)) GO TO 460
               IF (IM(IH,I).GT.IM(IH,K)) GO TO 420
  440       CONTINUE
         END IF
  460    IRANK(J) = I
         J = K
...
C
  ...
  620       K = K1
            K1 = IRANK(K)
            IF (K1.NE.0) THEN
               IF (A.EQ.IM(M1,K1)) THEN
                  DO 640 IH = M1 + 1, M2
                     IF (IM(IH,KTEMP).NE.IM(IH,K1)) GO TO 660
  640             CONTINUE
                  GO TO 620
               END IF
            END IF
