...
C
            ...
            IF (A.GE.C) THEN
               IF (A.EQ.C) THEN
                  DO 20 IH = N1 + 1, N2
                     IF (RM(K,IH).GT.RM(J,IH)) GO TO 40
                     IF (RM(K,IH).LT.RM(J,IH)) GO TO 60
   20             CONTINUE
               END IF
   40          IRANK(J) = K
               J = K
   ...
   60       IF (A.LE.B) THEN
               IF (A.EQ.B) THEN
                  DO 80 IH = N1 + 1, N2
                     IF (RM(K,IH).LT.RM(I,IH)) GO TO 100
                     IF (RM(K,IH).GT.RM(I,IH)) GO TO 120
   80             CONTINUE
                  GO TO 120
               END IF
  100          IRANK(K) = I
...
C
  ...
  140          I1 = I2
               I2 = IRANK(I1)
               IF (A.GT.RM(I2,N1)) GO TO 140
               IF (A.EQ.RM(I2,N1)) THEN
                  DO 160 IH = N1 + 1, N2
                     IF (RM(K,IH).LT.RM(I2,IH)) GO TO 180
                     IF (RM(K,IH).GT.RM(I2,IH)) GO TO 140
  160             CONTINUE
                  GO TO 140
               END IF
  180          CONTINUE
...
C
  220    ILIST = -1
         ...
         J = -LIST2
  240    K = I
         IF (RM(I,N1).GT.RM(J,N1)) K = J
         IF (RM(I,N1).EQ.RM(J,N1)) THEN
            DO 260 IH = N1 + 1, N2
               IF (RM(I,IH).GT.RM(J,IH)) GO TO 280
               IF (RM(I,IH).LT.RM(J,IH)) GO TO 300
  260       CONTINUE
            GO TO 300
  280       K = J
         END IF
...
C
  ...
  340    I = K
         K = IRANK(I)
         IF (K.LE.0) GO TO 480
         IF (A.GT.RM(K,N1)) GO TO 340
         IF (A.EQ.RM(K,N1)) THEN
            DO 360 IH = N1 + 1, N2
               IF (RM(J,IH).LT.RM(K,IH)) GO TO 380
               IF (RM(J,IH).GT.RM(K,IH)) GO TO 340
  360       CONTINUE
            GO TO 340
         END IF
  380    IRANK(I) = J
  ...
  420    J = K
         K = IRANK(J)
         IF (K.LE.0) GO TO 500
         IF (A.GT.RM(K,N1)) GO TO 420
         IF (A.EQ.RM(K,N1)) THEN
            DO 440 IH = N1 + 1, N2
               IF (RM(I,IH).LT.RM(K,IH)) GO TO 460
               IF (RM(I,IH).GT.RM(K,IH)) GO TO 420
  440       CONTINUE
         END IF
  460    IRANK(J) = I
         J = K
...
C
  ...
  620       K = K1
            K1 = IRANK(K)
            IF (K1.NE.0) THEN
               IF (A.EQ.RM(K1,N1)) THEN
                  DO 640 IH = N1 + 1, N2
                     IF (RM(KTEMP,IH).NE.RM(K1,IH)) GO TO 660
  640             CONTINUE
                  GO TO 620
               END IF
            END IF
