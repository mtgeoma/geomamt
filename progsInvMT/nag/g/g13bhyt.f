      SUBROUTINE G13BHY(ST,NST,NP,ND,NQ,NPS,NDS,NQS,NS,PHI,THETA,SPHI,
     *                  STHETA,NPARQ,C,RMS,NFV,FVA,FSD,KFVA,AEX,AAL,
     *                  AEXR)
C     MARK 11 RELEASE. NAG COPYRIGHT 1983.
C     MARK 11.5(F77) REVISED. (SEPT 1985.)
C
C     THIS SUBROUTINE CARRIES OUT THE CALCULATIONS INVOLVED
C     IN FORECASTING
C
C     .. Scalar Arguments ..
      DOUBLE PRECISION  C, RMS
      INTEGER           KFVA, ND, NDS, NFV, NP, NPARQ, NPS, NQ, NQS, NS,
     *                  NST
C     .. Array Arguments ..
      DOUBLE PRECISION  AAL(NST), AEX(NST), AEXR(NST), FSD(NFV),
     *                  FVA(NFV), PHI(NPARQ), SPHI(NPARQ), ST(NST),
     *                  STHETA(NPARQ), THETA(NPARQ)
C     .. Local Scalars ..
...
C
      ...
      D = C
C
C     BYPASS DERIVATION OF FORECAST VALUES IF KFVA=0
C
      IF (KFVA.EQ.0) GO TO 300
C
...
C        NEW VALUES AT END OF AEX, AND AT SUBDIVISION POINT IN AAL
C        AND AEXR
C
