...
C     .. Array Arguments ..
     ...
     *                  WK((IP*IP+3*IP+22)/2), WT(*), X(LDX,M), Y(N)
      INTEGER           ISX(M)
C     .. Local Scalars ..
      DOUBLE PRECISION  ACC, EPSA, TI, TOLA
      INTEGER           I, IERROR, IFAULT, ITER, J, K, MAXITA, NO, NREC
      LOGICAL           SVD
C     .. Local Arrays ..
      CHARACTER*80      P01REC(2)
C     .. External Functions ..
      DOUBLE PRECISION  G02GCV, X02AJF
      INTEGER           P01ABF
      EXTERNAL          G02GCV, X02AJF, P01ABF
C     .. External Subroutines ..
      EXTERNAL          F06FBF, G02GBS, G02GBT, G02GBZ, G02GCU, G02GCW,
     *                  G02GCX, G02GCY
...
C
  ...
  160    CONTINUE
     ...
     *       V(1,7),1)
         ...
         CALL G02GCU(N,LINK,A,Y,V(1,2),V(1,1),WT,NO,1.0D0,1.0D0)
         CALL G02GBZ(G02GCY,G02GCX,G02GCV,G02GCW,LINK,MEAN,WEIGHT,N,X,
     ...
     *                  V(1,4),V(1,6),WK)
            IF (WEIGHT.EQ.'U' .OR. WEIGHT.EQ.'u') THEN
               DO 180 I = 1, N
                  IF (Y(I).GT.V(I,2)) THEN
                     V(I,5) = SQRT(G02GCV(V(I,2),Y(I),TI))
                  ELSE
                     V(I,5) = -SQRT(G02GCV(V(I,2),Y(I),TI))
                  END IF
  180          CONTINUE
            ELSE
               DO 200 I = 1, N
                  IF (WT(I).GT.0.0D0) THEN
                     IF (Y(I).GT.V(I,2)) THEN
                        V(I,5) = SQRT(WT(I)*G02GCV(V(I,2),Y(I),TI))
                     ELSE
                        V(I,5) = -SQRT(WT(I)*G02GCV(V(I,2),Y(I),TI))
                     END IF
                  ELSE
...
99992 FORMAT (' ** On entry, IP incompatible with number of non-zero v',
     *       'alues of ISX:',/'              IP = ',I16)
99991 FORMAT (' ** On entry, A = 0.0 and LINK = E')
99990 FORMAT (' ** On entry, LINK is not valid : LINK = ',A1)
99989 FORMAT (' ** On entry, Y(',I16,').lt.0.0 ')
...
99982 FORMAT (' ** Fitted value is at the boundary')
