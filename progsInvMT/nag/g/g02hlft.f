...
C     .. Subroutine Arguments ..
      EXTERNAL          UCV
C     .. Local Scalars ..
      DOUBLE PRECISION  BMAX, CC, CX, SDU, SDV, SDW, SMAX, SU, SV, SW,
     *                  T, TEMP, U, UD, UMAX, W, WD, XN, ZNR
...
C
  ...
  220                CONTINUE
                     IJ = IJ + 1
                     COV(IJ) = (WK(M+I)*WK(M+I)-ZNR*ZNR/DBLE(M))*U +
     *                         COV(IJ)
  240             CONTINUE
                  DO 260 J = 1, M
...
C
            ...
            END IF
            SMAX = 0.0D0
            CC = SDU/DBLE((M+2)*M) + SU/DBLE(M)
            IF (CC.EQ.0.0D0) THEN
               GO TO 500
C
            ELSE
               T = (2.0D0*SU+SDU)/DBLE(M) - SDV
               IF (T.EQ.0.0D0) THEN
                  GO TO 480
C
               ELSE
                  ...
                  DO 320 I = 1, M
                     IF (I.NE.1) THEN
                        ...
                        DO 300 J = 1, I1
                           IJ = IJ + 1
                           COV(IJ) = -MIN(MAX(COV(IJ)/CC,-BL),BL)
                           SMAX = MAX(SMAX,ABS(COV(IJ)))
  300                   CONTINUE
                     END IF
                     IJ = IJ + 1
                     CX = -MIN(MAX((COV(IJ)/(2.0D0*CC)+T),-BD),BD)
  ...
  420                      CONTINUE
                           DO 460 J = 6, M
                              ...
                              DO 440 JJ = 1, JL
                                 JJ1 = (JJ-1)*5 + J1
                                 JJ2 = MIN(J2,JJ1+4)
                                 WRITE (REC,FMT=99985) (A(KK),KK=JJ1,
     *                             JJ2)
                                 CALL X04BAF(NOUT,REC)
  440                         CONTINUE
  460                      CONTINUE
...
C
            ...
            GO TO 520
C
  480       WRITE (EREC(1),FMT=99977)
            IERROR = 6
            GO TO 600
C
  500       WRITE (EREC(1),FMT=99978)
            IERROR = 6
            GO TO 600
C
C     CALCULATE INV(A)
...
C
  580       IERROR = 3
         ...
         END IF
      END IF
  600 IFAIL = P01ABF(IFAIL,IERROR,SRNAME,NREC,EREC)
C
...
99992 FORMAT (' ** On entry, BL.le.0.0 : BL =',D13.5)
99991 FORMAT (' ** On entry, BD.le.0.0 : BD =',D13.5)
...
99981 FORMAT (' ** W value returned by UCV.lt.0.0 : W(',D13.5,') =',
     *       D13.5)
99980 FORMAT (' ** The sum D3 is zero')
99979 FORMAT (' ** ',I16,' TH column of X has constant value')
99978 FORMAT (' ** The sum D1 is zero')
99977 FORMAT (' ** The sum D2 is zero')
      END
