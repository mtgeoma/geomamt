...
int main(int argc, char* argv[]) {
    ...
    string sstr="";
    string mode_type;
    string EFresp1, EFresp2;
    
    if (argc<5 || argc>6) {  // wrong number of parameters
        cerr<<"usage: "<<argv[0]<<" -M<mode type> -E<EF resp1>/<EF resp2> -H<header data> -I<input data> [-S<static_shift>]"<<endl
            <<"<mode type>           : Mode Type (tm, te or tp)"<<endl
            <<"<EF resp1>/<EF resp2> : Error floor for data response 1 and 2"<<endl
            <<"<header data>         : 1st line: station location"<<endl
            <<"                        2nd line: target periods"<<endl
        ...
        for(int i=1; i< argc; i++) {
            if(argv[i][0]=='-' && argv[i][1]=='I')
                ...
                mode_type=argv[i]+2;
                if(mode_type!="te" && mode_type!="tm" && mode_type!="tp") {
		    cerr<<"mode type in -M must be te, tm or tp."<<endl;
                    exit(1);
                }
            }
            else if(argv[i][0]=='-' && argv[i][1]=='E') {
                string ef=argv[i]+2;
                EFresp1=ef.substr(0,ef.find("/"));
                EFresp2=ef.substr(ef.find("/")+1,string::npos);
            }
    ...
    for(int s=0;s<ns;s++) {
	  ...
	  cout<<"number of periods in station "<<s<<" is bigger than number of periods in "<<hstr<<endl;
        ...
        for(int t=0;t<nT;t++) {
            getline(ifile,line);
            data datum=read_line_mackie(line,mode_type);
    ...
    if(mode_type=="tp") {
        cout<<"DATA_RESPONSE_NO_1* rel"<<endl;
        for(int t=0;t<T.size();t++) {
            for(int s=0;s<st.size();s++) {
                if(st[s][t].T!=0.)
                cout<<setw(12)<<st[s][t].rho;
              else
                cout<<setw(12)<<0.;
            }
            cout<<endl;
        ...
        cout<<"ERROR_RESPONSE_NO_1* "<<EFresp1<<endl;
        for(int t=0;t<T.size();t++) {
            for(int s=0;s<st.size();s++) {
	      if(st[s][t].T!=0.)
        ...
        cout<<"DATA_RESPONSE_NO_2* img"<<endl;
        for(int t=0;t<T.size();t++) {
            for(int s=0;s<st.size();s++) {
	      if(st[s][t].T!=0.)
	        cout<<setw(12)<<st[s][t].phi;
              else
	        cout<<setw(12)<<0.;
            }
            cout<<endl;
    ...
    cout<<"ERROR_RESPONSE_NO_2* "<<EFresp2<<endl;
    for(int t=0;t<T.size();t++) {
        for(int s=0;s<st.size();s++) {
            if(st[s][t].T!=0.)
	        cout<<setw(12)<<st[s][t].err;
