...
C     6 7 8     
     ...
     >                      area,iml,irl,ipl,isl,npp,nss,ipp,iss)
      ...
      REAL*8  min_dist,dist,dss,dpp,MAXFLOAT
...
c     Returns len if iwrd'th word is not found.
      character HORIZONTAL_TAB, SPACE, COMMA
      integer i, nword
      logical wasblk
      intrinsic len

      HORIZONTAL_TAB = achar(9)
      SPACE = achar(32)
      COMMA = achar(44)
      wasblk=.true.
      nword= 0
     ...
     &        string(i:i).eq.HORIZONTAL_TAB .or.
     &        string(i:i).eq.COMMA )then
