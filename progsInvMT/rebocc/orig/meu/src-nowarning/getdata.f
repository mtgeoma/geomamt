...
C                                                                    C
     ...
     >        REQUIRED_RMS,SD_RMS,STARTING_LM,
     >        STEPSIZE_LM,SMOOTH_SZLM,
...
700     CALL IncorrectInput('initial_iterno      ')
      ...
      IF (FindStr(ctmp,'logfile_screen').GT.0) THEN
        ...
        IF (FindStr(ccom,'no').GT.0) THEN
           ...
           GOTO 100
        ENDIF
        CALL IncorrectInput('logfile_screen      ')
      ENDIF
...
c1000  FORMAT('*** Absolute Error Floor of ',f5.2,' is assigned for ',
c     >       'the log10 app resistivity ***')
c1010  FORMAT('*** Absolute Error Floor of ',f5.2,' deg is assigned ',
c     >       ' for the phase ***')
...
C 
      SUBROUTINE SetSmooth(NMode,NSta,Nzb,Ny,ZDis,YDis,StaPos,
     >           DTIME_STEP,DLENGTH_HOR,DLENGTH_VER,HGamma,VGamma)
      INCLUDE 'parameter.h'
      INCLUDE 'constant.h'

      INTEGER DTIME_STEP,NMode,NSta(*),Ny,Nzb
c      INTEGER StaNod(NMODMX,NSTAMX)
