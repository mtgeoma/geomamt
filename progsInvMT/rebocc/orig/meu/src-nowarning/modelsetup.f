...
C
      SUBROUTINE LocateModelPosition(NMode,NSta,
     >           LStart,StaLoc,Nzb,Ny0,Dy0,
     >           Ny,Dy,Cy,YDis,IniRho,PriRho,StaPos,StaNod,
     >           ModGrd,DFStatus)
      INCLUDE 'parameter.h'

C        IO Variables
      INTEGER NMode,NSta(*),Ny0,Ny,Nzb
      INTEGER ModGrd(NZ0MX,NY0MX)
      INTEGER StaNod(NMODMX,NSTAMX)
      INTEGER DFStatus(*)
      REAL*8  Dy0(*),Dy(*),YDis(*),Cy(*)
      REAL*8  LStart(*)
c      REAL*8  Period(NMODMX,NPERMX)
...
c1110  FORMAT('????    NEW NY = ',i3,'   OLD NY = ',i3,'  ????')
